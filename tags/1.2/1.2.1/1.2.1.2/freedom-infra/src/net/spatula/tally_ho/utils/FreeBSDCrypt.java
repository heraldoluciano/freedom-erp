...
public class FreeBSDCrypt {
	...
	public static String crypt( String password, String salt ) {
		...
		/*
		 * First get the salt into a proper format. 
		 * It can be no more than 8 characters, 
		 * and if it starts with the magic string, 
		 ...
		 * first character of the second MD5 digest using the 
		 * smaller of the MD5 length or password length as the 
		 * number of bytes to use in the update.
		 */
		for ( int i = pwLength; i > 0; i -= md5Size ) {
			md5_1.update( md5_2_digest, 0, i > md5Size ? md5Size : i );
		}
		/*
		 * the FreeBSD code does a memset to 0 on "final" (md5_2_digest) 
		 * here which may be a bug, since it references "final" again 
		 * if the conditional below is true, meaning it always is equal to 0
		 */
		md5_2.reset();
		/*
		 * Again, update the first MD5 a few times, this time using 
		 * either 0 (see above) or the first byte of the password, 
		 * depending on the lowest order bit's value
		 */
		 ...
		 * is introduced to slow things down. 
		 * It also further mutates the result.
		 */
