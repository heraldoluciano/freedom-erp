	...
	public boolean imprimir( ResultSet rs, ResultSet rsRec, ImprimeOS imp ) {
		...
		try {
			...
			while ( rs.next() ) {
				...
				/*
				 * else { Vector vDesc = new Vector(); vDesc.addElement(Funcoes.strToVectorSilabas(rs.getString("ObsItVenda")==null || rs.getString("ObsItVenda").equals("") ? (rs.getString("DescProd").trim()):rs.getString("ObsItVenda"),45));
				 * vDesc.addElement(Funcoes.strDecimalToStrCurrency(13,2,rs.getString("VlrLiqItVenda"))); if (vDesc!=null) { vDescServ.addElement(vDesc.clone()); }
				 * 
