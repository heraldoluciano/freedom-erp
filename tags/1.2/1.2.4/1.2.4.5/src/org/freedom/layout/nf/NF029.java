...
public class NF029 extends Layout {
	...
	public boolean imprimir( NF nf, ImprimeOS imp ) {
		...
		try {
			...
			while ( itens.next() ) {
				...
				imp.pulaLinha( 1, imp.comprimido() );
				imp.say( 5, Funcoes.alinhaCentro( itens.getInt( NF.C_CODPROD ), 5 ) );
				imp.say( 11, Funcoes.copy( itens.getString( NF.C_DESCPROD ).trim(), 23 ) );
				imp.say( 65, sSigla );
				...
				if ( ( iItImp == itens.getInt( NF.C_CONTAITENS ) ) || ( imp.pRow() == MAXLINE - 2 ) ) {
					...
					imp.say( 91, frete.getString( NF.C_TIPOFRETE ).equals( "C" ) ? "1" : "2" );
