...
public class JBemaFI32 {
	...
	/*
	 * public native int bTEFStatus(String Identificacao); public native int bTEFVendaCartao(String Identificacao, String ValorCompra); public native int bTEFConfirmaVenda(String Identificacao, String ValorCompra, String Header); public native int bTEFNaoConfirmaVendaImpressao(String Identificacao,
	 * String ValorCompra); public native int bTEFCancelaVendaCartao(String Identificacao, String ValorCompra, String Nsu, String NumeroCupom, String Hora, String Data, String Rede); public native int bTEFImprimeTEF(String Identificacao, String FormaPagamento, String ValorCompra); public native int
	 * bTEFImprimeRelatorio(); public native int bTEFADM(String Identificacao); public native int bTEFVendaCompleta(String Identificacao, String ValorCompra, String FormaPagamento, String Texto); public native int bTEFConfiguraDiretorioTef(String PathREQ, String PathRESP); public native int
	 * bTEFVendaCheque(String Identificacao, String ValorCompra); public native int bTEFApagaResiduos();
	 */
	// Outras Funções
	...
	public String mp40fiCoord( int iPosVlrX, int iPosExt1X, int iPosExt2X, int iPosFavX, int iPosLocalX, int iPosDiaX, int iPosMesX, int iPosAnoX, int iPosVlrY, int iPosExt1Y, int iPosExt2Y, int iPosFavY, int iPosLocalY ) {
	...
	public boolean mp40fiCheque( String sUserID, String sCodbanc, String sNome, String sCidade, Date dtCheq, double deVlrCheque, boolean bModoDemo ) {
		...
		try {
			...
			while ( !trataRetornoFuncao( bImprimeCheque( sCodbanc, sVlrCheq, sNome, sCidade, sDataCheq, "" ) ) ) {
				Logger.gravaLogTxt( "", sUserID, Logger.LGEP_IMPRES_CHEQUE, "Banco: " + sCodbanc + "-Valor: " + sVlrCheq + "-Fav.: " + sNome + "-Cid: " + sCidade + "-Data: " + sDataCheq + "-" + sMensErroLog );
				if ( i >= 5 ) {
					bRetorno = false;
					break;
				}
	...
	/*
	 * public native int bRecebimentoNaoFiscal(String IndiceTotalizador, String Valor, String FormaPagamento); public native int bAbreComprovanteNaoFiscalVinculado(String FormaPagamento, String Valor, String NumeroCupom); public native int bUsaComprovanteNaoFiscalVinculado(String Texto); public
	 * native int bFechaComprovanteNaoFiscalVinculado();
	 */
		...
		try {
			if ( !bModoDemo ) {
				...
				while ( !trataRetornoFuncao( bVendeItem( sCodprod, sDescprod, sTributo, "F", sQuant, 2, sValor, "$", sValdesc ) ) ) {
					Logger.gravaLogTxt( "", sUserID, Logger.LGEP_IMPRES_ITEM, "Codigo: " + sCodprod + "-Descrição: " + sDescprod + "-Qtd.: " + sQuant + "-Valor: " + sValor + "-Desconto: " + sValdesc + "-Erro: " + sMensErroLog );
					Funcoes.espera( 2 );
					if ( i++ >= 5 ) {
						bRetorno = false;
						break;
		...
		try {
			if ( !bModoDemo ) {
				...
				while ( !trataRetornoFuncao( bFechaCupom( sFormaPagto, sAcreDesc, sTipoAcreDesc, sVlrAcreDesc, sVlrPago, sMensagem ) ) ) {
					Logger.gravaLogTxt( "", sUserID, Logger.LGEP_FECHA_CUPOM, "Forma Pagto.: " + sFormaPagto + " - Valor Pago:" + sVlrPago + " - Msg. final de cupom: " + sMensagem + " - Erro: " + sMensErroLog );
					Funcoes.espera( 2 );
					if ( i >= 5 ) {
						bRetorno = false;
						break;
