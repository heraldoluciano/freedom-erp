...
public class NF2001 extends Layout {
		...
		try {
			...
			while (itens.next()) {
				...
				if (!bTotalizou) {
					...
					vValores.addElement(String.valueOf((itens.getFloat(NF.C_VLRLIQPED) - frete.getFloat(NF.C_VLRFRETEPED) + itens.getFloat(NF.C_VLRADICITPED) - itens.getFloat(NF.C_VLRIPIPED))));//6
			...
			if(bMensage) {
				if(Funcoes.mensagemConfirma(null,
						"Podem conter erros na disposição da nota.\n"+
	...
	private String[] getDadosEmp() {
		...
		try {
			...
			sSQL = "SELECT RAZFILIAL, ENDFILIAL, NUMFILIAL, FONEFILIAL, FAXFILIAL, "
				 + "BAIRFILIAL, CIDFILIAL, UFFILIAL, CEPFILIAL "
				 + "FROM SGFILIAL "
