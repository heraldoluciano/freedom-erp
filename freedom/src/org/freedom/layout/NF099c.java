		...
		try {
			...
			while (itens.next()) {  
				...
				if (imp.pRow()==0) {
					...
					imp.say(  8, Funcoes.copy(cab.getString(NF.C_ENDEMIT),0,50).trim()+", "+(Funcoes.copy(String.valueOf(cab.getInt(NF.C_NUMEMIT)),0,6).trim()).trim()+" - "+(cab.getString(NF.C_COMPLEMIT) != null ? Funcoes.copy(cab.getString(NF.C_COMPLEMIT),0,9).trim() : "").trim());
				...
				if ((iItImp == itens.getInt(NF.C_CONTAITENS)) || (imp.pRow() == numMax)) {       	 
					if (iItImp == itens.getInt(NF.C_CONTAITENS)) {
						...
						frete.next();
						imp.pulaLinha(3, imp.comprimido());
						imp.say(  8, Funcoes.strDecimalToStrCurrency(20,2,String.valueOf(itens.getFloat(NF.C_VLRBASEICMSPED))));
