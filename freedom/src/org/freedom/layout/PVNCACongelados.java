		...
		try {
			...
			while (rs.next()) {
				...
				if (imp.pRow() == 0) {
					...
					imp.say(102, "Data: " + Funcoes.sqlDateToStrDate(rs.getDate("DTEMITVENDA")));
					imp.say(121, "Hora: " + sHora);
					imp.say(135, " |");
					imp.pulaLinha(1, imp.comprimido());
					imp.say(  1, "|");
					imp.say(  4, "Endereço: " + (rs.getString("ENDCLI") != null ? (rs.getString("ENDCLI").trim() +
					...
					imp.say( 104, "Fone/Fax: " + (rs.getString("FONECLI") != null ? Funcoes.setMascara(rs.getString("FONECLI").trim(),"####-####") : "") + " / " +
				...
				if(rs.getInt(1) == contItem || imp.pRow() == maxLine) {
					...
					imp.say( 54, "|OBS.: " + ((rs.getString("OBSCLI") != null && !rs.getString("OBSCLI").trim().equals("")) ? 
