    ...
    try {
     ...
     while (itens.next()) {
         ...
         if (imp.pRow()==0) {
	           ...
	           imp.say(imp.pRow()+1,0,"");
	           imp.say(imp.pRow()+1,0,"");
	           imp.say(imp.pRow()+0,5,sNat[0]);
	           imp.say(imp.pRow()+0,52,sNat[1]);
	           imp.say(imp.pRow()+1,0,"");
	           imp.say(imp.pRow()+1,0,"");
	           imp.say(imp.pRow()+1,0,""+imp.comprimido());
	           imp.say(imp.pRow()+0,5,cab.getInt(NF.C_CODEMIT)+" - "+cab.getString(NF.C_RAZEMIT));
	           ...
	           imp.say(imp.pRow()+1,0,"");
	           imp.say(imp.pRow()+1,0,""+imp.comprimido());
	           imp.say(imp.pRow()+0,5,Funcoes.copy(cab.getString(NF.C_ENDEMIT),0,50).trim()+", "+(!cab.getString(NF.C_NUMEMIT).equals("") ? Funcoes.copy(cab.getString(NF.C_NUMEMIT),0,6).trim() : "").trim()+" - "+(!cab.getString(NF.C_COMPLEMIT).equals("") ? Funcoes.copy(cab.getString(NF.C_COMPLEMIT),0,9).trim() : "").trim());
	           ...
	           imp.say(imp.pRow()+1,0,"");
	           imp.say(imp.pRow()+1,0,""+imp.comprimido());
	           imp.say(imp.pRow()+0,5,cab.getString(NF.C_CIDEMIT));
	           imp.say(imp.pRow()+0,53,(!cab.getString(NF.C_DDDEMIT).equals("") ? "("+cab.getString(NF.C_DDDEMIT)+")" : "")+
					   				   (!cab.getString(NF.C_FONEEMIT).equals("") ? Funcoes.setMascara(cab.getString(NF.C_FONEEMIT).trim(),"####-####") : "").trim());
	           imp.say(imp.pRow()+0,75,cab.getString(NF.C_UFEMIT));
         ...
         imp.say(imp.pRow()+0,78,Funcoes.copy(itens.getString(NF.C_ORIGFISC),0,1)+Funcoes.copy(itens.getString(NF.C_CODTRATTRIB),0,2));
         imp.say(imp.pRow()+0,86,itens.getString(NF.C_CODUNID).substring(0,4));
         imp.say(imp.pRow()+0,92,""+itens.getFloat(NF.C_QTDITPED));
         imp.say(imp.pRow()+0,105,Funcoes.strDecimalToStrCurrency(8,2,""+((new BigDecimal(itens.getFloat(NF.C_VLRLIQITPED))).divide(new BigDecimal(itens.getFloat(NF.C_QTDITPED)),2,BigDecimal.ROUND_HALF_UP))));
         imp.say(imp.pRow()+0,115,Funcoes.strDecimalToStrCurrency(13,2,""+itens.getFloat(NF.C_VLRLIQITPED)));
         imp.say(imp.pRow()+0,132,Funcoes.strDecimalToStrCurrency(3,2,""+itens.getFloat(NF.C_PERCICMSITPED)));
...
//         System.out.println(imp.pRow()+" = iItImp : "+iItImp);
         if ((iItImp == itens.getInt(NF.C_CONTAITENS)) || (imp.pRow() >= iLinMaxItens)) {
           ...
           if (iItImp == itens.getInt(NF.C_CONTAITENS)) {
	             ...
	             imp.say(imp.pRow()+1,0,"");
	             imp.say(imp.pRow()+1,0,""+imp.comprimido());
	             imp.say(imp.pRow()+0,5,Funcoes.strDecimalToStrCurrency(20,2,""+frete.getFloat(NF.C_VLRFRETEPED)));
           ...
           imp.say(imp.pRow()+0,72,frete.getString(NF.C_TIPOFRETE).equals("C") ? "1" : "2");
           imp.say(imp.pRow()+0,77,frete.getString(NF.C_PLACAFRETE));
           imp.say(imp.pRow()+0,93,frete.getString(NF.C_UFFRETE));
