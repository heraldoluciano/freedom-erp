	...
	public boolean carregaTabelas(Connection con, Vector parans ) {
		...
		try {
			sql = "SELECT V.CODVENDA, V.CODCLI, C.RAZCLI, C.CNPJCLI, C.CPFCLI, C.ENDCLI, C.NUMCLI, C.COMPLCLI, C.BAIRCLI, C.CEPCLI, "+
					...
					"C.CODSETOR,"+//"(SELECT S.DESCSETOR FROM VDSETOR S WHERE S.CODSETOR=C.CODSETOR AND S.CODFILIAL=C.CODFILIALSR AND S.CODEMP=C.CODEMPSR)," +
					"ITPG.DIASPAG "+
					"FROM VDVENDA V, VDCLIENTE C, FNPLANOPAG PG, FNPARCPAG ITPG, VDVENDEDOR VEND "+
					...
					"AND ITPG.CODEMP=PG.CODEMP AND ITPG.CODFILIAL=PG.CODFILIAL AND ITPG.CODPLANOPAG=PG.CODPLANOPAG "+
			...
			while (rs.next()) {
				...
				cab.setString(C_CODSETOR, (rs.getString(50)!=null ? rs.getString(50).trim() : ""));
