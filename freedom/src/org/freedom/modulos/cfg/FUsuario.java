...
public class FUsuario extends FTabDados implements PostListener, DeleteListener, 
		InsertListener, CarregaListener, ActionListener, CheckBoxListener {
...
private Connection conIB = null;
	...
	public FUsuario () {
		...
		adicDB(cbAltParc, 7, 30, 225, 20, "AltParcVenda", "", false);
		adicDB(cbAbreGaveta, 7, 50, 225, 20, "AbreGavetaUsu", "", false);
		adicDB(cbAlmoxarife, 7, 70, 225, 20, "AlmoxarifeUsu", "", false);
		adicDB(cbCompra, 7, 90, 225, 20, "ComprasUsu", "", false);
		
		txtCodAlmox.setRequerido(cbAlmoxarife.isSelected());
		
		adicCampo(txtCodAlmox, 7, 130, 80, 20, "CodAlmox", "Cód.almox.", ListaCampos.DB_FK, false);
		adicDescFK(txtDescAlmox, 90, 130, 170, 20, "DescAlmox", "Descrição do almoxarifado");
		adicDB(rgAprovaSolicitacao, 7, 190, 210, 80, "AprovCPSolicitacaoUsu", "Aprova solicitação", false);
		adicDB(rgAprovaRMA, 230, 190, 210, 80, "AprovRMAUsu", "Aprova RMA", false);
	...
	private void carregaAcesso() {
		...
		String sSQL = null;
		try {
			sSQL = "SELECT FL.CODFILIAL,FL.NOMEFILIAL FROM SGFILIAL FL, SGACESSOEU AC WHERE "+
