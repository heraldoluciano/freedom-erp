	...
	private void montaRel() {
		...
		try {
		  String sSQL = "SELECT "+
						...
						"(SELECT ATM.DOCATENDO FROM ATATENDIMENTO  ATM, ATATENDIMENTOORC AC WHERE " +
						"ATM.CODEMP=AC.CODEMP AND ATM.CODFILIAL=AC.CODFILIAL AND ATM.CODATENDO=AC.CODATENDO" +
						" AND O.CODEMP=AC.CODEMPOC AND O.CODFILIAL=AC.CODFILIALOC AND O.CODORC=AC.CODORC " +
						"AND O.TIPOORC=AC.TIPOORC),"+
						(comRef() ? "IT.REFPROD," : "IT.CODPROD,")+
                        "O.VLRDESCITORC, O.CODORC,C.CODCONV,C.NOMECONV,O.DTORC,O.DTVENCORC,C.CIDCONV,C.UFCONV,C.ENDCONV,C.BAIRCONV,C.FONECONV," +
		  	            " T.DESCTPCONV,O.OBSORC,O.VLRLIQORC,P.CODBARPROD, P.DESCPROD,IT.QTDITORC,IT.VLRDESCITORC,IT.VLRLIQITORC," +
		  	            " O.VLRDESCORC,  O.VLRADICORC, O.VLRPRODORC, F.CIDFILIAL, CL.RAZCLI,CL.CIDCLI,C.NUMCONV, " +
						"(SELECT NOMEENC FROM ATENCAMINHADOR EC WHERE EC.CODEMP=C.CODEMPEC AND EC.CODFILIAL=C.CODFILIAL AND EC.CODENC=C.CODENC)," +
						"(SELECT CIDENC FROM ATENCAMINHADOR EC WHERE EC.CODEMP=C.CODEMPEC AND EC.CODFILIAL=C.CODFILIAL AND EC.CODENC=C.CODENC)" +
		  	            " FROM VDORCAMENTO O, ATCONVENIADO C, ATTIPOCONV T, VDITORCAMENTO IT, EQPRODUTO P, SGFILIAL F, VDCLIENTE CL,FNPLANOPAG PL "+
...
//Fecha a conta e passa a regua:
	...
	private int montaCab(ResultSet rs) {
	  ...
	  try {
		...
		setFonte(fnCabEmpNeg);
		drawTexto(Funcoes.strZero(""+iCodOrc,8),65,iY);
		setFonte(fnCabEmp);
		...
		setFonte(fnCabEmpNeg);
		drawTexto(Funcoes.strZero(""+(rs.getInt(4)),8),190,iY);
