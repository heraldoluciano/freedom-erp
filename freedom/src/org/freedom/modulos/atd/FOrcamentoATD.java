...
public class FOrcamentoATD extends FDetalhe implements PostListener, CarregaListener, FocusListener, ActionListener, InsertListener, DeleteListener {
	...
	private JTextFieldPad txtPrecoItOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtPercDescItOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 6, 2 );

	private JTextFieldPad txtVlrDescItOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtVlrLiqItOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtVlrEdDescOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtVlrEdAdicOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtVlrDescOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtVlrAdicOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtVlrLiqOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtVlrProdItOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );

	private JTextFieldPad txtObsItOrc = new JTextFieldPad( JTextFieldPad.TP_STRING, 50, 0 );

	private JTextFieldPad txtVlrProdOrc = new JTextFieldPad( JTextFieldPad.TP_NUMERIC, 15, casasDecFin );
	...
	private void calcTot() {
		...
		BigDecimal bVlrProd = txtVlrProdItOrc.getVlrBigDecimal().subtract( txtVlrDescItOrc.getVlrBigDecimal() ).divide( new BigDecimal( "1" ), 3, BigDecimal.ROUND_HALF_UP );
	...
	public void focusLost( FocusEvent fevt ) {
		...
		if ( fevt.getSource() == txtPercDescItOrc ) {
			if ( txtPercDescItOrc.getText().trim().length() < 1 ) {
				...
				txtVlrDescItOrc.setVlrBigDecimal( txtVlrProdItOrc.getVlrBigDecimal().multiply( txtPercDescItOrc.getVlrBigDecimal() ).divide( new BigDecimal( "100" ), 3, BigDecimal.ROUND_HALF_UP ) );
				calcVlrProd();
				calcTot();
	...
	public void actionPerformed( ActionEvent evt ) {
		...
		if ( evt.getSource() == btFechaOrc ) {
			DLCompOrc dl = new DLCompOrc( this, ( txtVlrDescItOrc.getVlrBigDecimal().intValue() > 0 ), txtVlrProdOrc.getVlrBigDecimal(), txtVlrEdDescOrc.getVlrBigDecimal(), txtVlrEdAdicOrc.getVlrBigDecimal(), txtCodPlanoPag, txtDescPlanoPag );
