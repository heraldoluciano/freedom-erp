...
public class FRCentroCusto extends FRelatorio {
	...
	public void imprimir( boolean bVisualizar ) {
		...
		String sSQL = "SELECT CM.CODCC,CM.SIGLACC,CM.DESCCC,CM.NIVELCC,"
				+
				// Primeira sub-select >>
				"(SELECT SUM(SL.VLRSUBLANCA*-1) FROM FNSUBLANCA SL, FNLANCA L, FNPLANEJAMENTO PL" + " WHERE L.FLAG IN " + AplicativoPD.carregaFiltro( con, org.freedom.telas.Aplicativo.iCodEmp ) + ( sCodPlan.equals( "" ) ? "" : " AND L.CODPLAN=?" ) + " AND L.CODEMP=CM.CODEMP" + " AND L.CODFILIAL=?"
				...
				+ " WHERE L.FLAG IN " + AplicativoPD.carregaFiltro( con, org.freedom.telas.Aplicativo.iCodEmp ) 
