  		...
  		try {
  			...
  			while ( rs.next() ) {
  				...
  				if (imp.pRow()==0) {
  					...
  					imp.say(imp.pRow()+1,0,""+imp.comprimido());
  					imp.say(imp.pRow()+0,0,"|");
  					imp.say(imp.pRow()+0,(136-sFiltro.length())/2,sFiltro);
  					imp.say(imp.pRow()+0,136,"|");
  					imp.say(imp.pRow()+1,0,""+imp.comprimido());
  					imp.say(imp.pRow()+0,0,"|"+Funcoes.replicate("=",134)+"|");
  					imp.say(imp.pRow()+1,0,""+imp.comprimido());
  					imp.say(imp.pRow()+0,0,"|");
  					imp.say(imp.pRow()+0,(136-sVendedor.length())/2,sVendedor);
  					imp.say(imp.pRow()+0,136,"|");
  					imp.say(imp.pRow()+1,0,""+imp.comprimido());
  					imp.say(imp.pRow()+0,0,"|"+Funcoes.replicate("=",134)+"|");
  					imp.say(imp.pRow()+1,0,""+imp.comprimido());
  				...
  				if (rs.getDouble("VLRVENDACOMI")!=0)
  					dePercComi = rs.getDouble("VLRCOMI")*100/rs.getDouble("VLRPARCITREC");
  			    else 
  			    	dePercComi = 0;
  				imp.say(imp.pRow()+1,0,""+imp.comprimido());
  				imp.say(imp.pRow()+0,0,"|"+Funcoes.adicionaEspacos(rs.getString("RAZCLI"),25));
  				imp.say(imp.pRow()+0,27,"|"+Funcoes.adicEspacosEsquerda(rs.getString("DOCREC")+"-"+rs.getString("NPARCITREC"),10));
  				imp.say(imp.pRow()+0,38,"|"+Funcoes.adicEspacosEsquerda(rs.getInt("CODVENDA")+"",8));
  				imp.say(imp.pRow()+0,47,"|"+rs.getString("TIPOCOMI"));
  				imp.say(imp.pRow()+0,49,"|"+rs.getString("STATUSCOMI"));
  				imp.say(imp.pRow()+0,52,"|"+Funcoes.dateToStrDate(rs.getDate("DATACOMI")));
  				imp.say(imp.pRow()+0,63,"|"+Funcoes.dateToStrDate(rs.getDate("DTVENCCOMI")));
  				imp.say(imp.pRow()+0,74,"|"+Funcoes.strDecimalToStrCurrency(12,2,rs.getString("VLRPARCITREC")));
  				imp.say(imp.pRow()+0,87,"|"+Funcoes.strDecimalToStrCurrency(9,2,rs.getString("VLRCOMI")));
  				imp.say(imp.pRow()+0,98,"|"+Funcoes.strDecimalToStrCurrency(5,2,dePercComi+""));
  				imp.say(imp.pRow()+0,104,"|"+Funcoes.strDecimalToStrCurrency(9,2,rs.getString("VLRPAGOCOMI")));
  				imp.say(imp.pRow()+0,114,"|"+Funcoes.strDecimalToStrCurrency(9,2,rs.getString("VLRAPAGCOMI")));
  			...
  			imp.say(imp.pRow()+0,74,"|"+Funcoes.strDecimalToStrCurrency(12,2,""+deVlrVenda));
  			imp.say(imp.pRow()+0,87,"|"+Funcoes.strDecimalToStrCurrency(9,2,""+deVlrComi));
			imp.say(imp.pRow()+0,98,"|");
  			imp.say(imp.pRow()+0,104,"|"+Funcoes.strDecimalToStrCurrency(9,2,""+deVlrPago));
  			imp.say(imp.pRow()+0,114,"|"+Funcoes.strDecimalToStrCurrency(9,2,""+deVlrAPag));
...
//      	dl.dispose();
  			...
  			Funcoes.mensagemErro(this,"Erro consulta tabela de commissões!"+err.getMessage());      
  		}
