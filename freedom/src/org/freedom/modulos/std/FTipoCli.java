...
public class FTipoCli extends FDados implements ActionListener {
	...
	private void imprimir(boolean bVisualizar) {
		...
		try {
			...
			sSQL = "SELECT TP.CODTIPOCLI,TP.DESCTIPOCLI," +
				   "(SELECT COUNT(CLI.CODCLI) FROM VDCLIENTE CLI " +
				   							 "WHERE CLI.CODEMPTC=TP.CODEMP AND CLI.CODFILIALTC=TP.CODFILIAL " +
				   							 "AND CLI.CODTIPOCLI = TP.CODTIPOCLI)"+
				   "FROM VDTIPOCLI TP " +
			...
			while ( rs.next() ) {
				...
				imp.say(imp.pRow() + 1, 0, imp.normal());
				imp.say(imp.pRow(), 2, rs.getString("CodTipoCli"));
				imp.say(imp.pRow(), 20, rs.getString("DescTipoCli"));
