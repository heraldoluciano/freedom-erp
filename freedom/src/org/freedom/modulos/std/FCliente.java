...
public class FCliente extends FTabDados implements RadioGroupListener, PostListener, ActionListener, 
	...
	private void carregaTabHist(){
		...
		try {
			sSQL = "SELECT H.CODHISTTK,H.SITHISTTK,H.TIPOHISTTK,H.DATAHISTTK,H.DESCHISTTK," +
				   "H.CODCTO,H.CODATEND,A.NOMEATEND,H.HORAHISTTK " +
	...
	private Object[] getAgente() {
		...
		String sSQL = null;
		try {
			sSQL = "SELECT U.CODAGE,U.TIPOAGE FROM SGUSUARIO U " +
	...
	private void getContatos(){
		...
		try {
			...
			sSql = "SELECT EXTRACT(MONTH FROM TK.DATAHISTTK),COUNT(1) "+
				   ...
				   "AND CODEMPCL=? AND CODFILIALCL=? AND CODCLI=? " +
	...
	private boolean[] getPrefere() {
		...
		try {
			...
			bRet[2] = false;
			bRet[3] = true;
			sSQL = "SELECT SETORVENDA,RGCLIOBRIG,CLIMESMOCNPJ,CNPJOBRIGCLI FROM SGPREFERE1 " +
	...
	private void imprimir(boolean bVisualizar) {
		...
		if (sValores[15].length() > 0) {
			sWhere += " AND C1.CODCLASCLI="+sValores[15];
			sWhere2 += " AND C2.CODCLASCLI="+sValores[15];
		...
		if (sValores[7].equals("C")) {
			...
			sSQL = "SELECT C1.CODCLI,C1.RAZCLI,C1.PESSOACLI,C1.NOMECLI,C1.CONTCLI,C1.ENDCLI,C1.NUMCLI," +
				   "C1.BAIRCLI,C1.CIDCLI,C1.COMPLCLI,C1.UFCLI,C1.CEPCLI,C1.CNPJCLI,C1.INSCCLI,C1.CPFCLI," +
				   "C1.RGCLI,C1.FONECLI,C1.DDDCLI,C1.FAXCLI,C1.EMAILCLI,C1.CODPESQ " + sObs + 
				   "FROM VDCLIENTE C1 " + sFrom +
				   "WHERE C1.CODEMP=? AND C1.CODFILIAL=? " + sWhere + 
				   " ORDER BY " + sValores[0];
			...
			try {
				...
				while ( rs.next() ) {
					...
					imp.say( 80, "Tel:");
					imp.say( 86, (rs.getString("DDDCli") != null ? rs.getString("DDDCli")+"-" : "")+
			...
			sSQL = "SELECT C1.CODCLI,C1.RAZCLI,C1.ENDCLI,C1.CIDCLI," +
				   "C1.FONECLI,C1.DDDCLI,C1.CODPESQ " + sObs +
				   "FROM VDCLIENTE C1 " + sFrom +
				   "WHERE C1.CODEMP=? AND C1.CODFILIAL=? " + sWhere +
			...
			try {
				...
				while ( rs.next() ) {
					...
					imp.say(117, "|");
					imp.say(120, (rs.getString("DDDCli") != null ? "("+rs.getString("DDDCli")+")" : "")+
								 (rs.getString("FoneCli") != null ? Funcoes.setMascara(rs.getString("FoneCli").trim(),"####-####") : "").trim());
					imp.say(135,"|");
			...
			try {
				...
				while ( rs.next() ) {
					...
					imp.say(117, "|");
					imp.say(119, (rs.getString("FoneCli") != null ? ((rs.getString("DDDCli") != null ? "("+rs.getString("DDDCli")+")" : "") +
					...
					imp.say(117, "|");
					imp.say(119, (rs.getString("FaxCli") != null ? ((rs.getString("DDDCli") != null ? "("+rs.getString("DDDCli")+")" : "") +
			...
			/*sSQL = "SELECT C1.CODPESQ,C1.RAZCLI RAZMATRIZ,'A' TIPO,C1.CODCLI,C1.RAZCLI,C1.ENDCLI,C1.CIDCLI,C1.FONECLI " +
				   "FROM VDCLIENTE C1 " + sFrom + 
				   "WHERE C1.CODCLI=C1.CODPESQ AND C1.CODEMP=? AND C1.CODFILIAL=? " + sWhere + 
			       " UNION SELECT C2.CODPESQ,(SELECT C3.RAZCLI FROM VDCLIENTE C3 WHERE C3.CODCLI=C2.CODPESQ) RAZMATRIZ," +
			       "'B' TIPO,C2.CODCLI,C2.RAZCLI,C2.ENDCLI,C2.CIDCLI,C2.DDDCLI,C2.FONECLI " +
			       "FROM VDCLIENTE C2 " + sFrom + 
			       "WHERE NOT C2.CODCLI=C2.CODPESQ AND C2.CODEMP=? AND C2.CODFILIAL=? " + sWhere2 + 
			       " ORDER BY " + sOrdem;*/
			sSQL = "SELECT C1.CODPESQ,C1.RAZCLI RAZMATRIZ,'A' TIPO,C1.CODCLI,C1.RAZCLI,C1.ENDCLI,C1.CIDCLI,C1.FONECLI " +
				...
				if (( dl.OK ) && (!dl.getTexto().trim().equals(""))) {
					try {
						...
						sSql = "INSERT INTO VDOBSCLI (SEQOBSCLI, CODEMP, CODFILIAL, CODCLI, DTOBSCLI, HOBSCLI, TXTOBSCLI) " +
											 "VALUES ((COALESCE((SELECT MAX(OC.SEQOBSCLI) " +
											 		  "FROM VDOBSCLI OC " +
	...
	public void beforePost(PostEvent pevt) {
		...
		if ((lcCampos.getStatus() == ListaCampos.LCS_INSERT) && (duploCNPJ())) {
			if (bPref[2]) {
				if (Funcoes.mensagemConfirma(this,"Este CNPJ já está cadastrado! Salvar mesmo assim?")
