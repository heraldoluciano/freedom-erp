	...
	private void visualizar() {
		...
		try {
			String sSQL = "SELECT T.TIPOMOV, V.SERIE, V.DOCVENDA, V.DTEMITVENDA," + "VE.CODFORNVEND,P.CODFABPROD,IT.CODLOTE,SUM(IT.QTDITVENDA)," + "SUM(IT.VLRLIQITVENDA),SUM(" + "(SELECT SUM(IT2.VLRLIQITVENDA*(PARC.PERCPAG/100)*PARC.DIASPAG)"
					...
					+ " AND IT2.CODITVENDA = IT.CODITVENDA AND IT2.CODEMP=V2.CODEMP" + " AND IT2.CODFILIAL=V2.CODFILIAL AND PARC.CODPLANOPAG=V2.CODPLANOPAG" + " AND PARC.CODEMP=V2.CODEMPPG AND PARC.CODFILIAL=V2.CODFILIALPG))," + "C.CODTIPOCLI,C.CIDCLI,C.UFCLI,"
			...
			sSQL = sSQL + " GROUP BY " + "T.TIPOMOV, V.SERIE, V.DOCVENDA, V.DTEMITVENDA,VE.CODFORNVEND," + " P.CODFABPROD,IT.CODLOTE,C.CODTIPOCLI,C.CIDCLI,C.UFCLI," + "C.CEPCLI,C.CNPJCLI,C.CPFCLI,C.CODCLI,C.PESSOACLI,C.RAZCLI," + "C.ENDCLI,C.NUMCLI,C.COMPLCLI,C.BAIRCLI,C.ENDENT,C.NUMENT,"
					+ "C.COMPLENT,C.BAIRENT,C.ENDCOB,C.NUMCOB,C.COMPLCOB,C.BAIRCOB," + "C.CIDCOB,C.UFCOB,C.CEPCOB,C.EMAILCLI,C.CONTCLI,C.FONECLI," + "T.ESTIPOMOV,V.CODVENDA,IT.CODITVENDA,IT.CODPROD ";

			sSQL = sSQL + "UNION ALL " +

			"SELECT T.TIPOMOV, C.SERIE, C.DOCCOMPRA, C.DTEMITCOMPRA," + "CAST(NULL AS CHAR(13)),P.CODFABPROD,IT.CODLOTE,SUM(IT.QTDITCOMPRA)," + "SUM(IT.VLRLIQITCOMPRA),SUM(" + "(SELECT SUM(IT2.VLRLIQITCOMPRA*(PARC.PERCPAG/100)*PARC.DIASPAG)"
					+ " FROM CPCOMPRA C2, CPITCOMPRA IT2, FNPARCPAG PARC WHERE " + " C2.CODCOMPRA=C.CODCOMPRA" + " AND C2.CODEMP=C.CODEMP AND C2.CODFILIAL=C.CODFILIAL" + " AND IT2.CODCOMPRA=IT.CODCOMPRA" + " AND IT2.CODITCOMPRA = IT.CODITCOMPRA AND IT2.CODEMP=C2.CODEMP"
					+ " AND IT2.CODFILIAL=C2.CODFILIAL AND PARC.CODPLANOPAG=C2.CODPLANOPAG" + " AND PARC.CODEMP=C2.CODEMPPG AND PARC.CODFILIAL=C2.CODFILIALPG))," + "4,F.CIDFOR,F.UFFOR," + "F.CEPFOR,F.CNPJFOR,F.CPFFOR,F.CODFOR,F.PESSOAFOR,F.RAZFOR,F.ENDFOR,"
					+ "F.NUMFOR,F.COMPLFOR,F.BAIRFOR,F.ENDFOR,F.NUMFOR,F.COMPLFOR," + "F.BAIRFOR,F.ENDFOR,F.NUMFOR,F.COMPLFOR,F.BAIRFOR," + "F.CIDFOR,F.UFFOR,F.CEPFOR,F.EMAILFOR,F.CONTFOR,F.FONEFOR," + "T.ESTIPOMOV,C.CODCOMPRA,IT.CODITCOMPRA,IT.CODPROD," + " MIN((SELECT DINILOTE FROM EQLOTE L"
					+ " WHERE L.CODPROD=IT.CODPROD AND L.CODLOTE=IT.CODLOTE AND" + " L.CODEMP=IT.CODEMPLE AND L.CODFILIAL=IT.CODFILIALLE))" + " FROM CPCOMPRA C, CPITCOMPRA IT, CPFORNECED F, EQPRODUTO P," + " EQTIPOMOV T, SGPREFERE1 PF WHERE C.CODEMP=? AND C.CODFILIAL=? AND"
					+ " C.DTEMITCOMPRA BETWEEN ? AND ? AND IT.CODCOMPRA=C.CODCOMPRA AND" + " IT.CODEMP=C.CODEMP AND IT.CODFILIAL=C.CODFILIAL AND" + " F.CODFOR=C.CODFOR AND F.CODFILIAL=C.CODFILIALFR AND" + " F.CODEMP=C.CODEMPFR AND" + " P.CODPROD=IT.CODPROD AND P.CODEMP=IT.CODEMPPD AND"
					+ " P.CVPROD IN ('V','A') AND P.TIPOPROD='P' AND" + " P.CODFILIAL=IT.CODFILIALPD AND T.CODTIPOMOV=c.CODTIPOMOV AND" + " T.CODEMP=C.CODEMPTM AND T.CODFILIAL=C.CODFILIALTM AND T.ESTOQTIPOMOV='S' AND " + " ((P.CODMARCA=PF.CODMARCA AND P.CODEMPMC=PF.CODEMPMC AND"
					+ " P.CODFILIALMC=PF.CODFILIALMC) OR PF.CODMARCA IS NULL) AND" + " ((P.CODGRUP=PF.CODGRUP AND P.CODEMPGP=PF.CODEMPGP AND" + " P.CODFILIALGP=PF.CODFILIALGP) OR PF.CODGRUP IS NULL) AND" + " NOT (C.CODFOR=PF.CODFOR AND C.CODEMPFR=PF.CODEMPFR AND" + " C.CODFILIALFR=PF.CODFILIALFR)"
			...
			sSQL = sSQL + " GROUP BY " + " T.TIPOMOV, C.SERIE, C.DOCCOMPRA, C.DTEMITCOMPRA," + " P.CODFABPROD,IT.CODLOTE,F.CIDFOR,F.UFFOR," + "F.CEPFOR,F.CNPJFOR,F.CPFFOR,F.CODFOR,F.PESSOAFOR,F.RAZFOR," + "F.ENDFOR,F.NUMFOR,F.COMPLFOR,F.BAIRFOR,F.ENDFOR,F.NUMFOR,"
					+ "F.COMPLFOR,F.BAIRFOR,F.ENDFOR,F.NUMFOR,F.COMPLFOR,F.BAIRFOR," + "F.CIDFOR,F.UFFOR,F.CEPFOR,F.EMAILFOR,F.CONTFOR,F.FONEFOR," + "T.ESTIPOMOV,C.CODCOMPRA,IT.CODITCOMPRA,IT.CODPROD ";
