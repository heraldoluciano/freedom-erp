...
/*		rgTipo.setVlrString( "V" );
	 ...
	 // Mecanismo de busca de informações de tipo de crédito 
 	 ...
 	 // Adicionando elementos no painel superior da tela (Cabeçalho)
	    ...
	    adic(new JLabelPad("Tipo de crédito"), 7, 80, 120, 20);
		adic(txtCodTpCred, 7, 100, 50, 20);
		adicDescFK(txtDescTpCred, 60, 100, 197, 20, "DescTpCred", "");
		adicDescFK(txtVlrTpCred, 260, 100, 100, 20, "VlrTpCred", "Pré-aprovado");
	...
	private void calculaLiberacao() {
		try {
			...
			txtVlrALiberar.setVlrBigDecimal( 
					txtVlrTpCred.getVlrBigDecimal().subtract( 
							txtVlrAberto.getVlrBigDecimal().add( 
									txtVlrLiqPed.getVlrBigDecimal() ) ).multiply( new BigDecimal(-1) ) );
