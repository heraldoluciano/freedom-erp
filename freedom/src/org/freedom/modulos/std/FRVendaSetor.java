...
public class FRVendaSetor extends FRelatorio implements RadioGroupListener {
	...
	private String getColSetor(Vector vCols, int iTotPassadas, int iPassada) {
		...
		try {
			...
			sRetorno += Funcoes.replicate(" ", 102 - sRetorno.length()) + (iPassada == 0 ? "| TOTAL" : "|      ");
		} finally {
			vCols = null;
		}
		return sRetorno;
	...
	private String getTotSetor(Vector vTotSetor, int iTotPassadas, int iPassada) {
		...
		try {
			...
			sRetorno += Funcoes.replicate(" ", 102 - sRetorno.length()) + "|" + ((iTotPassadas - 1) == iPassada ? Funcoes
							.strDecimalToStrCurrency(11, 2, deTotal + "") : Funcoes.replicate(" ", 11)); 
		} finally {
			vTotSetor = null;
			deTemp = null;
			deTotal = 0;
		...
		try {			
			...
			try {
				...
				for (int i = 0; i < vItens.size(); i++) {
					...
					if ((!sCodSetor.equals(vItem.elementAt(POS_CODSETOR).toString()))
						...
						if (!sCodSetorAnt.equals("")) {
							...
							imp.say(imp.pRow(), 123, "|" + Funcoes.strDecimalToStrCurrency(11, 2,((Vector) vItens.elementAt(i - 1)).elementAt(POS_TOTSETOR).toString()));
					...
					if (iCol == 0) {
						...
						imp.say(imp.pRow(), 9, "| "	+ (vItem.elementAt(POS_SIGLAGRUP).equals("") ? 
								Funcoes.copy(vItem.elementAt(POS_CODGRUP).toString(), 10): vItem.elementAt(POS_SIGLAGRUP).toString()));
					}
				...
				if (iPos < vItens.size()) {
					...
					imp.say(imp.pRow(), 123, "|" + Funcoes .strDecimalToStrCurrency(11, 2,
											((Vector) vItens.elementAt(iPos)).elementAt(POS_TOTSETOR).toString()));
					imp.say(imp.pRow(), 135, "|");
					iLinsSetor++;
					iCol = 0;
				}
