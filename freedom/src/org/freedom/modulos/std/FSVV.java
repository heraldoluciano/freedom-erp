...
public class FSVV extends FFilho implements ActionListener {
  ...
  private void visualizar() {
    ...
    if (!txtDatainiLote.getVlrString().trim().equals("")) {
  	...
  	try {
      String sSQL = "SELECT T.TIPOMOV, V.SERIE, V.DOCVENDA, V.DTEMITVENDA," +
      		                "VE.CODFORNVEND,P.CODFABPROD,IT.CODLOTE,SUM(IT.QTDITVENDA)," +
      		                "SUM(IT.VLRLIQITVENDA),SUM(" +
                            "(SELECT SUM(IT2.VLRLIQITVENDA*(PARC.PERCPAG/100)*PARC.DIASPAG)" +
                            " FROM VDVENDA V2, VDITVENDA IT2, FNPARCPAG PARC WHERE " +
                            " V2.TIPOVENDA=V.TIPOVENDA AND V2.CODVENDA=V.CODVENDA" +
                            " AND V2.CODEMP=V.CODEMP AND V2.CODFILIAL=V.CODFILIAL" +
                            " AND IT2.TIPOVENDA=IT.TIPOVENDA AND IT2.CODVENDA=IT.CODVENDA" +
                            " AND IT2.CODITVENDA = IT.CODITVENDA AND IT2.CODEMP=V2.CODEMP" +
                            " AND IT2.CODFILIAL=V2.CODFILIAL AND PARC.CODPLANOPAG=V2.CODPLANOPAG" +
                            " AND PARC.CODEMP=V2.CODEMPPG AND PARC.CODFILIAL=V2.CODFILIALPG))," +
      		                "C.CODTIPOCLI,C.CIDCLI,C.UFCLI," +
      		                "C.CEPCLI,C.CNPJCLI,C.CPFCLI,C.CODCLI,C.PESSOACLI,C.RAZCLI,C.ENDCLI," +
      		                "C.NUMCLI,C.COMPLCLI,C.BAIRCLI,C.ENDENT,C.NUMENT,C.COMPLENT," +
      		                "C.BAIRENT,C.ENDCOB,C.NUMCOB,C.COMPLCOB,C.BAIRCOB," +
                            ...
                            " VE.CODEMP=V.CODEMPVD AND VE.CODFILIAL=V.CODFILIALVD AND" +
                            " P.CODPROD=IT.CODPROD AND P.CODEMP=IT.CODEMPPD AND" +
                            " P.CVPROD IN ('V','A') AND P.TIPOPROD='P' AND" +
      		                " P.CODFILIAL=IT.CODFILIALPD AND T.CODTIPOMOV=V.CODTIPOMOV AND" +
                            " T.CODEMP=V.CODEMPTM AND T.CODFILIAL=V.CODFILIALTM AND T.ESTOQTIPOMOV='S' AND " +
							...
							" AND PF.CODEMP=? AND PF.CODFILIAL=? GROUP BY " +
                            "T.TIPOMOV, V.SERIE, V.DOCVENDA, V.DTEMITVENDA,VE.CODFORNVEND," +
                            " P.CODFABPROD,IT.CODLOTE,C.CODTIPOCLI,C.CIDCLI,C.UFCLI," +
                            "C.CEPCLI,C.CNPJCLI,C.CPFCLI,C.CODCLI,C.PESSOACLI,C.RAZCLI," +
                            "C.ENDCLI,C.NUMCLI,C.COMPLCLI,C.BAIRCLI,C.ENDENT,C.NUMENT," +
							...
							"(SELECT SUM(IT2.VLRLIQITCOMPRA*(PARC.PERCPAG/100)*PARC.DIASPAG)" +
							" FROM CPCOMPRA C2, CPITCOMPRA IT2, FNPARCPAG PARC WHERE " +
							" C2.CODCOMPRA=C.CODCOMPRA" +
							" AND C2.CODEMP=C.CODEMP AND C2.CODFILIAL=C.CODFILIAL" +
							" AND IT2.CODCOMPRA=IT.CODCOMPRA" +
							" AND IT2.CODITCOMPRA = IT.CODITCOMPRA AND IT2.CODEMP=C2.CODEMP" +
							" AND IT2.CODFILIAL=C2.CODFILIAL AND PARC.CODPLANOPAG=C2.CODPLANOPAG" +
							" AND PARC.CODEMP=C2.CODEMPPG AND PARC.CODFILIAL=C2.CODFILIALPG))," +
							"4,F.CIDFOR,F.UFFOR," +
							"F.CEPFOR,F.CNPJFOR,F.CPFFOR,F.CODFOR,F.PESSOAFOR,F.RAZFOR,F.ENDFOR," +
							"F.NUMFOR,F.COMPLFOR,F.BAIRFOR,F.ENDFOR,F.NUMFOR,F.COMPLFOR," +
							"F.BAIRFOR,F.ENDFOR,F.NUMFOR,F.COMPLFOR,F.BAIRFOR," +
							"F.CIDFOR,F.UFFOR,F.CEPFOR,F.EMAILFOR,F.CONTFOR,F.FONEFOR," +
							"T.ESTIPOMOV,C.CODCOMPRA,IT.CODITCOMPRA,IT.CODPROD," +
      		                " MIN((SELECT DINILOTE FROM EQLOTE L" +
      		                " WHERE L.CODPROD=IT.CODPROD AND L.CODLOTE=IT.CODLOTE AND" +
      		                " L.CODEMP=IT.CODEMPLE AND L.CODFILIAL=IT.CODFILIALLE))" +
							" FROM CPCOMPRA C, CPITCOMPRA IT, CPFORNECED F, EQPRODUTO P," +
							" EQTIPOMOV T, SGPREFERE1 PF WHERE C.CODEMP=? AND C.CODFILIAL=? AND" +
							" C.DTEMITCOMPRA BETWEEN ? AND ? AND IT.CODCOMPRA=C.CODCOMPRA AND" +
							" IT.CODEMP=C.CODEMP AND IT.CODFILIAL=C.CODFILIAL AND" +
							" F.CODFOR=C.CODFOR AND F.CODFILIAL=C.CODFILIALFR AND" +
							" F.CODEMP=C.CODEMPFR AND" +
							" P.CODPROD=IT.CODPROD AND P.CODEMP=IT.CODEMPPD AND" +
							" P.CVPROD IN ('V','A') AND P.TIPOPROD='P' AND" +
							" P.CODFILIAL=IT.CODFILIALPD AND T.CODTIPOMOV=c.CODTIPOMOV AND" +
							" T.CODEMP=C.CODEMPTM AND T.CODFILIAL=C.CODFILIALTM AND T.ESTOQTIPOMOV='S' AND " +
							" ((P.CODMARCA=PF.CODMARCA AND P.CODEMPMC=PF.CODEMPMC AND" +
							" P.CODFILIALMC=PF.CODFILIALMC) OR PF.CODMARCA IS NULL) AND" +
							" ((P.CODGRUP=PF.CODGRUP AND P.CODEMPGP=PF.CODEMPGP AND" +
							" P.CODFILIALGP=PF.CODFILIALGP) OR PF.CODGRUP IS NULL) AND" +
                            " NOT (C.CODFOR=PF.CODFOR AND C.CODEMPFR=PF.CODEMPFR AND" +
                            " C.CODFILIALFR=PF.CODFILIALFR)" +
							" AND PF.CODEMP=? AND PF.CODFILIAL=? GROUP BY " +
							" T.TIPOMOV, C.SERIE, C.DOCCOMPRA, C.DTEMITCOMPRA," +
							" P.CODFABPROD,IT.CODLOTE,F.CIDFOR,F.UFFOR," +
							"F.CEPFOR,F.CNPJFOR,F.CPFFOR,F.CODFOR,F.PESSOAFOR,F.RAZFOR," +
							"F.ENDFOR,F.NUMFOR,F.COMPLFOR,F.BAIRFOR,F.ENDFOR,F.NUMFOR," +
      ...
      ps.setDate(9,Funcoes.dateToSQLDate(txtDataini.getVlrDate()));
      ps.setDate(10,Funcoes.dateToSQLDate(txtDatafim.getVlrDate()));
...
//Impressao do end. ent.          
        sTmp = rs.getString("EndEnt") != null ? rs.getString("EndEnt").trim() :
                                   (rs.getString("EndCli") != null ? rs.getString("EndCli").trim() : "");
        sTmp = sTmp.trim() + (rs.getString("NumEnt") != null ? ", "+rs.getString("NumEnt") : 
        	                       (rs.getString("NumCli") != null ? ", "+rs.getString("NumCli") : ""));
        sTmp = sTmp.trim() + (rs.getString("ComplEnt") != null ? " - "+rs.getString("ComplEnt").trim() : 
                                   (rs.getString("ComplCli") != null ? " - "+rs.getString("ComplCli").trim() : ""));
        tab.setValor(verifErro('B',Funcoes.copy(sTmp.trim(),40),sIdentC+"[Endereço]",""),i,21);
        sTmp = rs.getString("BairEnt") != null ? rs.getString("BairEnt").trim() :
                                   (rs.getString("BairCli") != null ? rs.getString("BairCli").trim() : "");
        tab.setValor(verifErro('B',sTmp,sIdentC+"[Bairro]",""),i,22);
//Impressao do end. cob.          
        sTmp = rs.getString("EndCob") != null ? rs.getString("EndCob") :
                                   (rs.getString("EndCli") != null ? rs.getString("EndCli").trim() : "");
        sTmp = sTmp.trim() + (rs.getString("NumCob") != null ? ", "+rs.getString("NumCob") :
        	                       (rs.getString("NumCli") != null ? ", "+rs.getString("NumCli") : ""));
        sTmp = sTmp.trim() + (rs.getString("ComplCob") != null ? " - "+rs.getString("ComplCob") : 
								   (rs.getString("ComplCli") != null ? " - "+rs.getString("ComplCli") : ""));
        tab.setValor(Funcoes.copy(sTmp.trim(),40),i,23);
        sTmp = rs.getString("BairCob") != null ? rs.getString("BairCob").trim() :
		                           (rs.getString("BairCli") != null ? rs.getString("BairCli").trim() : "");
        tab.setValor(sTmp,i,24);
        sTmp = rs.getString("CidCob") != null ? rs.getString("CidCob").trim() :
		                           (rs.getString("CidCli") != null ? rs.getString("CidCli").trim() : "");
        tab.setValor(sTmp,i,25);
        sTmp = rs.getString("UfCob") != null ? rs.getString("UfCob").trim() :
		                           (rs.getString("UfCli") != null ? rs.getString("UfCli").trim() : "");
        tab.setValor(sTmp,i,26);
        sTmp = rs.getString("CepCob") != null ? rs.getString("CepCob").trim() :
        	                       (rs.getString("CepCli") != null ? rs.getString("CepCli").trim() : "");
        tab.setValor(sTmp,i,27);
        tab.setValor(rs.getString("EmailCli") != null ? rs.getString("EmailCli").trim() : "" ,i,28);
        tab.setValor(rs.getString("ContCli") != null ? rs.getString("ContCli").trim() : "" ,i,29);
      ...
      if (fArq.createNewFile()) {
        ...
        if (cbEstoque.getVlrString().equals("S")) {
            ...
            String sSQL = "SELECT P.CODFABPROD,P.CODPROD,MAX(MP.CODMOVPROD),"+
                          ...
                          "FROM EQMOVPROD MP, EQPRODUTO P,SGPREFERE1 PF "+
                          "WHERE MP.CODEMP=? AND MP.CODFILIAL=? AND "+
                          "P.CVPROD IN ('V','A') AND P.TIPOPROD='P' AND " +
                          "MP.DTMOVPROD<=? AND P.CODPROD=MP.CODPROD AND "+
                          "P.CODEMP=MP.CODEMPPD AND P.CODFILIAL=MP.CODFILIALPD AND "+
                          "((P.CODMARCA=PF.CODMARCA AND P.CODEMPMC=PF.CODEMPMC AND "+
                          "P.CODFILIALMC=PF.CODFILIALMC) OR PF.CODMARCA IS NULL) AND "+
                          "((P.CODGRUP=PF.CODGRUP AND P.CODEMPGP=PF.CODEMPGP AND "+
                          "P.CODFILIALGP=PF.CODFILIALGP) OR PF.CODGRUP IS NULL) AND "+
