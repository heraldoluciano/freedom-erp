...
public class FRVendasCFOP extends FRelatorio{
	...
	public void imprimir(boolean bVisualizar) {
		...
		try {
			sSQL = "SELECT V.CODVENDA, V.DOCVENDA, V.DTEMITVENDA, V.DTSAIDAVENDA, "
			...
			while ( rs.next() ) {
				...
				if (imp.pRow()==0) {
					...
					imp.say(imp.pRow(), 0, "|" + Funcoes.replicate("-",133) + "|");   
					imp.say(imp.pRow()+1, 0, imp.comprimido());
					imp.say(imp.pRow(), 0, "| NF");
					imp.say(imp.pRow(), 14, "| EMISSÃO");
					imp.say(imp.pRow(), 27, "| SAIDA");
					imp.say(imp.pRow(), 40, "| PEDIDO");
					imp.say(imp.pRow(), 50, "| COD.CLI.");
					imp.say(imp.pRow(), 61, "| NOME DO CLIENTE");
					imp.say(imp.pRow(),114, "|      VLR. TOTAL");
				...
				if (!rs.getString("CODNAT").equals(sCFOP)) {
					...
					imp.say(imp.pRow()+1, 0, imp.comprimido());
					imp.say(imp.pRow(), 0, "|");
					imp.say(imp.pRow(), 7, sCFOP + " - " + rs.getString("DESCNAT"));
					imp.say(imp.pRow(),135, "|");
					imp.say(imp.pRow()+1, 0, imp.comprimido());
					imp.say(imp.pRow(), 0, "|" + Funcoes.replicate("-",133) + "|");
				}
				...
				imp.say(imp.pRow()+1, 0, imp.comprimido());		
				imp.say(imp.pRow(), 0, "| " + Funcoes.copy(rs.getString("DocVenda"),0,10));
				imp.say(imp.pRow(), 14, "| " + Funcoes.sqlDateToStrDate(rs.getDate("DtEmitVenda")));
				imp.say(imp.pRow(), 27, "| " + Funcoes.sqlDateToStrDate(rs.getDate("DtSaidaVenda")));
				imp.say(imp.pRow(), 40, "| " + rs.getInt("CodVenda"));
				imp.say(imp.pRow(), 50, "| " + rs.getInt("CodCli"));
				imp.say(imp.pRow(), 61, "| " + Funcoes.copy(rs.getString("RazCli"),0,50));
				imp.say(imp.pRow(),114, "| " + Funcoes.strDecimalToStrCurrency(15,2,""+rs.getFloat(9)));
				imp.say(imp.pRow(),135, "|");
			...
			imp.say(imp.pRow()+1, 0, imp.comprimido());
			imp.say(imp.pRow(), 0, "|");
			imp.say(imp.pRow(), 96, "TOTAL GERAL       | " + Funcoes.strDecimalToStrCurrency(15,2,"" +bTotalGeral));
			imp.say(imp.pRow(),135,"|");
			imp.say(imp.pRow()+1, 0, imp.comprimido());
			imp.say(imp.pRow(), 0, "+" + Funcoes.replicate("=",133) + "+");
