...
public class DLRemIndustria extends FFDialogo implements MouseListener, ActionListener {
	...
	private JTextFieldFK txtRazFor = new JTextFieldFK( JTextFieldPad.TP_STRING, 50, Aplicativo.casasDec );

	private JTextFieldPad txtCodTipoMov = new JTextFieldPad( JTextFieldPad.TP_INTEGER, 8, 0 );

	private JTextFieldFK txtDescTipoMov = new JTextFieldFK( JTextFieldPad.TP_STRING, 50, Aplicativo.casasDec );

	private JTextFieldPad txtCodPlanoPag = new JTextFieldPad( JTextFieldPad.TP_INTEGER, 8, 0 );

	private JTextFieldFK txtDescPlanoPag = new JTextFieldFK( JTextFieldPad.TP_STRING, 50, Aplicativo.casasDec );
	...
	private enum enumTabRemessa {
		...
		SEL, SEQ, CODPROD, REFPROD, DESCPROD, QTD, CODVENDA  
	...
	public DLRemIndustria( DbConnection con, Integer pcodop, Integer pseqop ) {
		...
		setTitulo( "Remaessa para industrialização" );
	...
	public void carregaTabela( ) {
		...
		sql.append("it.refprodpd, it.codprodpd, pd.descprod, it.qtditop, pd.codalmox, it.codvenda ");
		...
		sql.append("it.codemp=op.codemp and it.codfilial=op.codfilial and it.codprod=op.codprod and it.seqest=op.seqest and ");
		sql.append("pd.codemp=it.codemppd and pd.codfilial=it.codfilialpd and pd.codprod=it.codprodpd and ");
		...
		try {
			...
			while ( rs.next() ) {
				...
				Integer codprod = rs.getInt( "CODPRODPD" );
				...
				tabRemessa.setValor( rs.getString( "REFPRODPD" ), i, enumTabRemessa.REFPROD.ordinal() );
...
//			sql.append("SELECT ISEQ FROM SPGERANUM(? , ?, 'VD') " );
	...
	private void gerarItensRemessa(FVenda fvenda) {
