...
public class Tef {
    ...
    public Tef(String sPathEnv, String sPathRet) {
        ...
        if (!verifTef()) {
            Funcoes.mensagemInforma(null,
                    "Gerenciador Padrão de TEF não está ativo!");
    ...
    public boolean enviaArquivo(String sConteudo[]) {
        ...
        try {
            ...
            Funcoes.mensagemErro(null,
                    "Não foi possível gravar o arquivo temporário de TEF!");
            err.printStackTrace();
        }
        return bRet;
    }
    ...
    public boolean verifTef() {
        ...
        try {
            ...
            Funcoes.mensagemErro(null,
                    "Não foi possível verificar se o módulo TEF está ativo!");
            err.printStackTrace();
        }
        return bRet;
    }
	...
	public Properties solicVenda(int iNumCupom, BigDecimal bigVal) {
        ...
        bRet = enviaArquivo(new String[] {
                ...
                (TEF_VAL_TOTAL + " = " + (Funcoes.transValor(bigVal, 12, 2,
                        false)).trim()) });
        if (!bRet || !existeStatus("CRT", lIdent)
    ...
    public boolean confirmaAdm(Properties prop) {
        ...
        if (!bRet
                || !existeStatus("CNF", Long.parseLong(prop.getProperty(
                        TEF_IDENTIFICACAO, "0"))))
            return false;
