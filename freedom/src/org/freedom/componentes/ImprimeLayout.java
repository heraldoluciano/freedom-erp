...
public class ImprimeLayout extends JPanelPad implements Printable, VetoableChangeListener {
   ...
   * 
   *  @param bMonta - Se true monta o relatorio e false manda direto para impressora.
  ...
  /** 
   *  (Não mexa!!!)...só para saber: esta função é responsável por montar o relatório na hora que ele vai para a impressora.
   *  Não mexa aqui porque esta classe esta ligada com o printJob.
   * 
...
//	    Como a primeira tag é: RELATORIO eu baixo um nivel indo para: PAGINA_#:
...
//Ferramentas de desenho:
  ...
  public void drawLinha(int x, int y, int tam, int alt, byte al) {  //Linha alinhada.
  ...
  public void drawTexto(String str, int x, int y, int tam, byte al) {  //Texto alinhado.
  ...
  public void drawRetangulo(int x, int y, int larg, int alt) {  //Retangulo Simples.
  ...
  public void drawRetangulo(int x, int y, int larg, int alt,String sPreenche) {  //Retangulo que coisa dentro.
	Element eRetangulo = doc.createElement("RETANGULO");
	eRetangulo.setAttribute("x",""+x);
	eRetangulo.setAttribute("y",""+y);
	eRetangulo.setAttribute("larg",""+larg);
  ...
  public void drawRetangulo(int x, int y, int tam, int alt, byte al) {  //Retangulo alinhado.
  ...
  public void drawElipse(int x, int y, int larg, int alt,String sPreenche) {   //Elipse com coisa dentro.
  ...
  public void drawElipse(int x, int y, int larg, int alt,String sPreenche, byte al) {  //Elipse que tem tudo.
...
//	   chart.draw(gr,new Rectangle2D.Double(15,105,500,500));
  ...
  private void trataNoh(Graphics2D gr,Node noh) {
  	if (noh.getNodeName().equals("TEXTO")) {
		...
		if (nVal != null && (sVal = nVal.getNodeValue()) != null)
  		gr.drawString(
  		  sVal,
  		  Integer.parseInt(noh.getAttributes().getNamedItem("x").getNodeValue()),
  		  Integer.parseInt(noh.getAttributes().getNamedItem("y").getNodeValue())
  		);
  	}
	else if (noh.getNodeName().equals("FONTE")) {
		fnAtual = 
		    new Font(
		    noh.getAttributes().getNamedItem("nome").getNodeValue(),
		    Integer.parseInt(noh.getAttributes().getNamedItem("estilo").getNodeValue()),
		    Integer.parseInt(noh.getAttributes().getNamedItem("tamanho").getNodeValue())
		  );
		gr.setFont(fnAtual);		
	}
	else if (noh.getNodeName().equals("COR")) {
		cCorPreenc = new Color(Integer.parseInt(noh.getAttributes().getNamedItem("rCorPreenc").getNodeValue()),
					 		   Integer.parseInt(noh.getAttributes().getNamedItem("gCorPreenc").getNodeValue()),
					  		   Integer.parseInt(noh.getAttributes().getNamedItem("bCorPreenc").getNodeValue()) );
		cCorBorda = new Color(Integer.parseInt(noh.getAttributes().getNamedItem("rCorBorda").getNodeValue()),
							  Integer.parseInt(noh.getAttributes().getNamedItem("gCorBorda").getNodeValue()),
	  ...
	  gr.drawLine(
		Integer.parseInt(noh.getAttributes().getNamedItem("x1").getNodeValue()),
		Integer.parseInt(noh.getAttributes().getNamedItem("y1").getNodeValue()),
		Integer.parseInt(noh.getAttributes().getNamedItem("x2").getNodeValue()),
		Integer.parseInt(noh.getAttributes().getNamedItem("y2").getNodeValue())
	  );
	}
	else if (noh.getNodeName().equals("PINCEL")) {
	  gr.setStroke(
	    new BasicStroke(
		  Float.parseFloat(noh.getAttributes().getNamedItem("larg").getNodeValue()),
		  Integer.parseInt(noh.getAttributes().getNamedItem("ponta").getNodeValue()),
		  Integer.parseInt(noh.getAttributes().getNamedItem("juncao").getNodeValue()),
		  Float.parseFloat(noh.getAttributes().getNamedItem("vertice").getNodeValue())
		)
	  );
	}
	  ...
	  if (noh.getAttributes().getNamedItem("enc") !=null) 
	    if (noh.getAttributes().getNamedItem("enc").getNodeValue().equals("S")) {
	      gr.fillRect(
		    Integer.parseInt(noh.getAttributes().getNamedItem("x").getNodeValue()),
		    Integer.parseInt(noh.getAttributes().getNamedItem("y").getNodeValue()),
		    Integer.parseInt(noh.getAttributes().getNamedItem("larg").getNodeValue()),
  	  ...
  	  case AL_BCEN:
		  ...
		  if (nVal != null && (sVal = nVal.getNodeValue()) != null) 
	  	  ...
	  	  iX[0] = Integer.parseInt(noh.getAttributes().getNamedItem("x").getNodeValue()) +				    	  	  		   
	  	  		  ((Integer.parseInt(noh.getAttributes().getNamedItem("tam").getNodeValue()) - 
	  	  		  tamStr)/2);	  	  		  
		  iX[1] = iX[0] + Integer.parseInt(noh.getAttributes().getNamedItem("tam").getNodeValue());
		  break; 
  ...
  /**
   *  Esta função ajusta a escala de zoom X/Y.
   *  A escala padrão é 1, isso aumenta ou diminui<BR>
   *  mudando a escala.<BR>
   *  por exemplo:  <BR> 
  ...
  /**
   *  Esta função grava o arquivo temporário XML. 
   *  Esta função deve ser chamada só depois que o <BR>
  ...
  /**
   *  Esta função será sobreposta pelas classes filhas.
   *  Esta função é sempre chamada pela classe DLPrintJob<BR>
...
class PaginaPad extends PageFormat {
  public PaginaPad(Paper pp) {
  	 pp.setImageableArea(
  	   pp.getImageableX()-50.0,
	   pp.getImageableY()-50.0,
	   pp.getImageableWidth()+100.0,
	   pp.getImageableHeight()+105.0
	 );  
  	 setPaper(pp);
  }
}