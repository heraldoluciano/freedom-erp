        ...
        try {
            ...
            if (sTipoUsoImp.equals("TO")) {
                ...
                sSQL = "SELECT EI."
                        + sPortaOS
                        + ",EI.IMPGRAFICA FROM SGESTACAOIMP EI,SGIMPRESSORA I "
                        + "WHERE EI.CODEST=? AND EI.TIPOUSOIMP='"
                        + sTipoUsoImp
                        + "' AND "
                        + "I.CODIMP=EI.CODIMP AND I.CODFILIAL=EI.CODFILIALIP AND "
                        + "I.CODEMP=EI.CODEMPIP AND EI.CODEMP=? AND EI.CODFILIAL=?";
        ...
        if (strTipoCab.compareTo("1") == 0) {
            ...
            if (borda) {
                ...
                for (int i = 0; i < subTitulos.size(); i++) {
                    ...
                    if (!sTmp.trim().equals("")) {
                        say(pRow() + 1, 0, "|");
                        say(pRow() + 0, ((iTamRel - sTmp.length() - 2) / 2),
                                sTmp);
                        say(pRow() + 0, iTamRel - 1, "|");
                    }
                }
                /*
                 * if (iTamRel == 80) say(pRow() + 1, 0, normal()); else
                 * say(pRow() + 1, 0, comprimido());
                 * 
                 * say(pRow() + 0, 0, "|"); say(pRow() + 0, iTamRel - 1, "|");
                 */
    ...
    public int verifLinPag(String sTipo) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                sClassNota = rs.getString("ClassNotaPapel") != null ? rs
                        .getString("ClassNotaPapel").trim() : "";
            } else {
                Funcoes.mensagemInforma(cOwner,
                        "Não foi encontrada nenhuma impressora do tipo '"
                                + sTipo + "!");
            }
            rs.close();
            ps.close();
            ...
            Funcoes.mensagemErro(cOwner,
                    "Erro ao consultar a tabela SGESTACAOIMP\n"
                            + err.getMessage());
            return 0;
        }
        return iRetorno;
    ...
    public boolean gravaPdf(File fArq) {
        ...
        try {
            ...
            for (int i = 0; i < getNumPags(); i++) {
                 ...
                 * Funcoes.mensagemErro(null,"Erro criando graphics:
                 * "+pe.getMessage()); }
                 */
                tp.setWidth(wt);
                tp.setHeight(ht);
