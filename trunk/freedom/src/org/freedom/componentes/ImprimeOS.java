...
public class ImprimeOS implements ActionListener {
	...
	public int getTipoImp() {
		...
		if ( sTipoUsoImp.equals( "TO" ) ) {
			sSQL = "SELECT I.CODIMP,I.TIPOIMP,I.DESCIMP FROM SGIMPRESSORA I, SGESTACAOIMP EI WHERE " + "EI.CODEST=? AND EI.CODEMP=? AND EI.CODFILIAL=? AND EI.IMPPAD='S' AND " + "EI.CODEMPIP=I.CODEMP AND EI.CODFILIALIP=I.CODFILIAL AND EI.CODIMP=I.CODIMP";
		}
		else {
			sSQL = "SELECT I.CODIMP,I.TIPOIMP,I.DESCIMP FROM SGIMPRESSORA I, SGESTACAOIMP EI WHERE " + "EI.CODEST=? AND EI.CODEMP=? AND EI.CODFILIAL=? AND " + "EI.TIPOUSOIMP='" + this.sTipoUsoImp + "' AND " + "EI.CODEMPIP=I.CODEMP AND EI.CODFILIALIP=I.CODFILIAL AND EI.CODIMP=I.CODIMP";
		}
	...
	public void setPortaImp() {
		...
		try {
			...
			if ( sTipoUsoImp.equals( "TO" ) ) {
				sSQL = "SELECT EI." + sPortaOS + ",EI.IMPGRAFICA FROM SGESTACAOIMP EI,SGIMPRESSORA I " + "WHERE EI.CODEST=? AND EI.IMPPAD='S' AND " + "I.CODIMP=EI.CODIMP AND I.CODFILIAL=EI.CODFILIALIP AND " + "I.CODEMP=EI.CODEMPIP AND EI.CODEMP=? AND EI.CODFILIAL=?";
			}
			else {
				sSQL = "SELECT EI." + sPortaOS + ",EI.IMPGRAFICA FROM SGESTACAOIMP EI,SGIMPRESSORA I " + "WHERE EI.CODEST=? AND EI.TIPOUSOIMP='" + sTipoUsoImp + "' AND " + "I.CODIMP=EI.CODIMP AND I.CODFILIAL=EI.CODFILIALIP AND " + "I.CODEMP=EI.CODEMPIP AND EI.CODEMP=? AND EI.CODFILIAL=?";
			}
			try {
				ps = con.prepareStatement( sSQL );
				ps.setInt( 1, Aplicativo.iNumEst );
	...
	public int verifLinPag( String sTipo ) {
		...
		if ( sTipo == null )
			sTipo = sTipoUsoImp;
		if ( sTipo.equals( "TO" ) ) {
			sSQL = "SELECT PP.LINPAPEL,PP.CLASSNOTAPAPEL FROM SGPAPEL PP, SGESTACAOIMP EI WHERE " + "EI.CODEST=? AND EI.CODEMP=? AND EI.CODFILIAL=? AND EI.IMPPAD='S' AND " + "EI.CODEMPPP=PP.CODEMP AND EI.CODFILIALPP=PP.CODFILIAL AND EI.CODPAPEL=PP.CODPAPEL";
		}
		else {
			sSQL = "SELECT PP.LINPAPEL,PP.CLASSNOTAPAPEL FROM SGPAPEL PP, SGESTACAOIMP EI WHERE " + "EI.CODEST=? AND EI.CODEMP=? AND EI.CODFILIAL=? AND " + "EI.TIPOUSOIMP='" + sTipo + "' AND " + "EI.CODEMPPP=PP.CODEMP AND EI.CODFILIALPP=PP.CODFILIAL AND EI.CODPAPEL=PP.CODPAPEL";
		}
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
