	...
	public boolean carregaTabelas(Connection con, Vector parans ) {
		...
		try {
			...
			sql = "SELECT I.CODITVENDA, I.CODPROD, P.REFPROD, P.DESCPROD, I.OBSITVENDA, P.CODUNID, " +
					...
					"V.VLRIPIVENDA, V.VLRLIQVENDA, N.IMPDTSAIDANAT, V.VLRPRODITVENDA, N.DESCNAT, N.CODNAT " +
			...
			sql = "SELECT I.DTVENCITREC,I.VLRPARCITREC,I.NPARCITREC " +
					...
					"WHERE R.CODVENDA=V.CODVENDA AND I.CODREC=R.CODREC ORDER BY I.DTVENCITREC" +
					"AND V.CODEMP=? AND V.CODFILIAL=? AND V.CODVENDA=? AND V.TIPOVENDA='V' ";
			ps = con.prepareStatement(sql);
			ps.setInt(1,((Integer) parans.elementAt(0)).intValue());
			ps.setInt(2,((Integer) parans.elementAt(1)).intValue());
			...
			sql = "SELECT T.CODTRAN, T.RAZTRAN, T.NOMETRAN, T.INSCTRAN, T.CNPMTRAN, T.TIPOTRAN, " +
					"T.ENDTRAN, T.NUMTRAN, T.CIDTRAN, T.UFTRAM , F.TIPOFRETEVD, F.PLACAFRETEVD, " +
					"F.UFFRETEVD, F.QTDFRETEVD, F.ESPFRETEVD, F.MARCAFRETEVD, F.PESOBRUTVD, F.PESOLIQVD, " +
					"V.VLRFRETEVENDA " +
					"FROM VDTRANSP T, VDFRETE F, VDVENDA V " +
					"WHERE T.CODEMP=F.CODEMPTN AND T.CODFILAL=F.CODFILIALTN AND T.CODTRAN=F.CODTRAN " +
