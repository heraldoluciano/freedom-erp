      ...
      while (rs.next()) {
         ...
         if (imp.pRow()==0) {
          ...
          // imp.say(imp.pRow()+1,0,"");
           ...
           imp.say(imp.pRow()+0,65,(rs.getString("DDDCli") != null ? "("+rs.getString("DDDCli")+")" : "")+Funcoes.setMascara(rs.getString("FoneCli").trim(),"####-####"));
           ...
           //imp.say(imp.pRow()+1,0,"");
          //imp.say(imp.pRow()+1,0,"");
           imp.say(imp.pRow()+1,0,"");imp.say(imp.pRow()+1,0,"");
           imp.say(imp.pRow()+1,0,""+imp.comprimido());		
  ...
  */                      
  ...
  private void impServ(Vector vServs,BigDecimal bigSomaServ,String sVlrIss,ImprimeOS imp) {
    ...
    if (vServs!=null) {
    	...
    	while (iImp<4) {
    		if (iImp==1) imp.say(imp.pRow()+1,120,Funcoes.strDecimalToStrCurrency(13,2,sVlrIss));
    		else if (iImp==3) imp.say(imp.pRow()+1,120,Funcoes.strDecimalToStrCurrency(13,2,bigSomaServ+""));
    		else imp.say(imp.pRow()+1,05,"");
    		iImp++;
    	}
    }
 }
