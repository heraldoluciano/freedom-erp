...
//					imp.say(imp.pRow()+0,94,"X");
					...
					imp.say(imp.pRow()+1,0,"");
					imp.say(imp.pRow()+1,20,Funcoes.copy(rs.getString("EndCli"),0,30).trim()+", "+(rs.getString("NumCli") != null ? Funcoes.copy(rs.getString("NumCli"),0,6).trim() : "").trim()+" - "+(rs.getString("ComplCli") != null ? Funcoes.copy(rs.getString("ComplCli"),0,9).trim() : "").trim());
...
//Descrições adicionais colocadas junto a decrição do produto.
				String[] sDesc = Funcoes.strToStrArray(rs.getString("ObsItVenda"),20);
				int iConta = 0;
				while(!sDesc[iConta].equals("") && iConta < 20) {
					if (iConta > 0) //Soh naum pula uma linha na primeira. 
						imp.say(imp.pRow()+1,0,""+imp.comprimido());
					imp.say(imp.pRow()+0,14,sDesc[iConta]);
					iConta++;
				}
				if (iConta == 0) //Se naum imprimiu nenhuma descadic: 
