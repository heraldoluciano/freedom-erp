...
public class FRRazCli extends FRelatorio {
	...
	public void imprimir( boolean bVisualizar ) {
		...
		try {
			...
			sSQL.append( "0 DOC, (COALESCE( ( SELECT SUM(R.VLRPARCREC) FROM FNRECEBER R WHERE " );
			...
			sSQL.append( "R.CODCLI=C.CODCLI AND R.CODEMP=? AND R.CODFILIAL=? AND L.DATALANCA < ? ), 0) ) VALOR " );
			...
			sSQL.append( "R.VLRPARCREC VALOR FROM FNRECEBER R, VDCLIENTE C WHERE C.CODEMP=R.CODEMPCL AND " );
			...
			sSQL.append( " 'R' TIPO, R.DOCREC DOC, (L.VLRLANCA * -1) VALOR FROM FNLANCA L, FNRECEBER R, VDCLIENTE C " );
