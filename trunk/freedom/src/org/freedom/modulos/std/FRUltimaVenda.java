  ...
  public void imprimir(boolean bVisualizar) {
    ...
    if (txtCodVend.getText().trim().length() > 0) {
		...
		sCab += sTmp+Funcoes.replicate(" ",133-sTmp.length())+" |";
            ...
            sSQL ="SELECT C.CODCLI,C.RAZCLI,C.FONECLI,VD.CODVENDA, VD.DOCVENDA, VD.VLRLIQVENDA, MAX(VD.DTEMITVENDA)" +
		          ...
		          "AND C.CODEMP=? "+sWhere+" GROUP BY C.CODCLI, C.RAZCLI,C.FONECLI,VD.CODVENDA,VD.DOCVENDA,VD.VLRLIQVENDA ";
    ...
    try {
      ...
      while ( rs.next() ) {
        ...
        imp.say(imp.pRow()+0,79,Funcoes.dateToStrDate(rs.getDate(7)));
...
//      dl.dispose();
		...
		Funcoes.mensagemErro(this,"Erro consulta ao relatório de vendas!"+err.getMessage());      
    }
