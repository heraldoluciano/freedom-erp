...
public class FRInadimplentes extends FRelatorio {
	...
	public void imprimir( boolean bVisualizar ) {
		...
		String sSQL = "SELECT IT.DTVENCITREC,IT.NPARCITREC,R.CODVENDA," + "R.CODCLI,C.RAZCLI,IT.VLRPARCITREC,C.FONECLI,C.DDDCLI," + "IT.DTITREC,(SELECT V.STATUSVENDA FROM VDVENDA V" + " WHERE V.FLAG IN " + AplicativoPD.carregaFiltro( con, org.freedom.library.swing.frame.Aplicativo.iCodEmp )
				+ " AND V.CODVENDA=R.CODVENDA)," + "R.DOCREC,R.CODREC,R.CODVENDA" + " FROM FNITRECEBER IT,FNRECEBER R,VDCLIENTE C" + " WHERE R.FLAG IN " + AplicativoPD.carregaFiltro( con, org.freedom.library.swing.frame.Aplicativo.iCodEmp ) + " AND IT.DTVENCITREC BETWEEN ? AND ? AND"
				+ " R.CODREC = IT.CODREC AND IT.STATUSITREC IN (" + sFiltro + ") AND" + " C.CODCLI=R.CODCLI AND R.CODEMPCL=C.CODEMP AND R.CODFILIALCL=C.CODFILIAL AND IT.CODEMP=R.CODEMP AND IT.CODFILIAL=R.CODFILIAL " + sWhere + " ORDER BY IT.DTVENCITREC,C.RAZCLI";
