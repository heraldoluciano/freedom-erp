...
public class FVenda extends FVD implements PostListener, CarregaListener, FocusListener, ActionListener, InsertListener, DeleteListener, KeyListener {
	...
	private void calcComisIt() {
		...
		if ( txtPercComItVenda.floatValue() >= 0 ) {
			if ( !(Boolean) oPrefs[ POS_PREFS.COMISSAODESCONTO.ordinal() ] ) {
				...
				txtVlrComisItVenda.setVlrBigDecimal( new BigDecimal( Funcoes.arredFloat( ( txtVlrProdItVenda.floatValue() - txtVlrDescItVenda.floatValue() ) * txtPercComItVenda.floatValue() / 100, casasDecFin ) ) );
			}
		}
	}
	...
	private void carregaComisIt() {
		...
		/**
		 * A ideia inicial era um sql unico com union e unico resultado porém não consegui aplicar um order by que trabalha-se da forma a qual seria necessária
		 */
	...
	private void imprimir( boolean bVisualizar, int iCodVenda ) {
		...
		try {
			...
			sSQL.append( "FROM VDCLIENTE C, EQPRODUTO P,FNPLANOPAG PG, VDITVENDA I " );
