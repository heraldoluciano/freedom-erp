		...
		try {
			...
			if ( cbSaida.getVlrString().equals( "S" ) ) {
				...
				sSQL = "SELECT V.TIPOVENDA, V.DTEMITVENDA,V.DTSAIDAVENDA,IV.CODNAT,"
						...
						+ "V.SERIE, L.PRIMCUPOMLX DOCVENDA, L.ULTCUPOMLX DOCFIM,IV.PERCICMSITVENDA, cast(coalesce(CF.ALIQLFISC,0) as decimal(15,5)) aliqfisc, IV.PERCIPIITVENDA,"
						+ "V.CODEMPCL,V.CODFILIALCL,IV.CODEMPNT,IV.CODFILIALNT,TM.CODEMPMN,TM.CODFILIALMN," 
						+ "SUM(IV.VLRLIQITVENDA) VLRCONTABILITVENDA,SUM(CASE WHEN (CF.TIPOFISC='TT' AND (CF.REDFISC<=0 OR CF.REDFISC IS NULL)) " 
						...
						+ "SUM(CASE WHEN CF.TIPOFISC='TT' AND (CF.REDFISC=0 OR CF.REDFISC IS NULL) " + "THEN IV.VLRLIQITVENDA*IV.PERCICMSITVENDA/100  WHEN CF.REDFISC>0 "
						+ "THEN ((IV.VLRLIQITVENDA)-(IV.VLRLIQITVENDA*CF.REDFISC/100))* CF.ALIQLFISC/100 ELSE NULL END) VLRICMSITVENDA, "
						+ "SUM(CASE WHEN (CF.TIPOFISC='II' AND ( CF.REDFISC<=0 OR CF.REDFISC IS NULL)) THEN IV.VLRLIQITVENDA WHEN CF.REDFISC>0 "
						...
						+ "ELSE NULL END ) ELSE NULL  END) AS VLRISENTASITVENDA, SUM(CASE WHEN (CF.TIPOFISC IN ('NN','FF')) THEN IV.VLRLIQITVENDA "
						+ "WHEN (CF.TIPOFISC IN ('II','TT')) OR (CF.REDFISC>0) OR (CF.REDFISC IS NOT NULL) "
						+ "THEN NULL ELSE IV.VLRLIQITVENDA END) VLROUTRASITVENDA, SUM(IV.VLRBASEIPIITVENDA) VLRBASEIPIITVENDA, "
						+ "SUM(IV.VLRIPIITVENDA) VLRIPIITVENDA, V.STATUSVENDA, V.VLRBASEICMSSTVENDA,V.VLRICMSSTVENDA,FT.ADICFRETEVD,FT.VLRFRETEVD "
						...
						+ "L.CODCAIXA=V.CODCAIXA AND L.DTLX=V.DTEMITVENDA " + "LEFT OUTER JOIN VDFRETEVD FT ON "
						+ "FT.CODEMP=V.CODEMP AND FT.CODFILIAL=V.CODFILIAL AND FT.TIPOVENDA=V.TIPOVENDA AND FT.CODVENDA=V.CODVENDA "
						...
						+ "SUBSTRING(V.STATUSVENDA FROM 1 FOR 1)<>'C' AND V.TIPOVENDA='E' AND V.DTEMITVENDA BETWEEN ? AND ? AND IV.CODEMP=V.CODEMP AND "
						+ "IV.CODVENDA=V.CODVENDA AND ( IV.CANCITVENDA IS NULL OR IV.CANCITVENDA<>'S' ) AND "
						+ "P.CODEMP=IV.CODEMPPD AND P.CODFILIAL=IV.CODFILIALPD AND P.CODPROD=IV.CODPROD AND "
						+ "CF.CODEMP=IV.CODEMPIF AND CF.CODFILIAL=IV.CODFILIALIF AND CF.CODFISC=IV.CODFISC AND CF.CODITFISC=IV.CODITFISC AND CF.GERALFISC='S' AND "
						+ "C.CODEMP=V.CODEMPCL AND C.CODFILIAL=V.CODFILIALCL AND C.CODCLI=V.CODCLI " 
						+ "GROUP BY V.TIPOVENDA,V.DTEMITVENDA, V.DTSAIDAVENDA, IV.CODNAT, V.CODCLI, C.UFCLI, TM.ESPECIETIPOMOV, TM.CODMODNOTA, "
						+ "V.SERIE, L.PRIMCUPOMLX, L.ULTCUPOMLX, IV.PERCICMSITVENDA, CF.ALIQLFISC, IV.PERCIPIITVENDA, "
						+ "V.CODEMPCL,V.CODFILIALCL,IV.CODEMPNT,IV.CODFILIALNT,TM.CODEMPMN,TM.CODFILIALMN, V.STATUSVENDA, "
						+ "V.VLRBASEICMSSTVENDA,V.VLRICMSSTVENDA,FT.ADICFRETEVD,FT.VLRFRETEVD "
						+ "ORDER BY 2,10";
