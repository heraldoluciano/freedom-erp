...
public class FTratTrib extends FDados implements ActionListener{
  ...
  private void imprimir(boolean bVisualizar) {
    ...
    String sSQL = "SELECT CODTRATTRIB,DESCTRATTRIB FROM LFTRATTRIB ORDER BY "+dl.getValor();
    ...
    try {
      ...
      while ( rs.next() ) {
         ...
         imp.say(imp.pRow()+1,0,""+imp.normal());
         imp.say(imp.pRow()+0,2,rs.getString("Codtrattrib"));
         imp.say(imp.pRow()+0,30,rs.getString("Desctrattrib"));
         if (imp.pRow()>=linPag) {
            imp.incPags();
            imp.eject();
         }
