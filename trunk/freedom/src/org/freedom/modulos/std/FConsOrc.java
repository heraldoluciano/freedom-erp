	...
	*/
	...
	private void carregaTabela() {
		...
		sSQL = "SELECT O.STATUSORC,O.CODORC,O.DTORC,O.DTVENCORC, " +
			 ...
			 "CL.CIDCLI,IT.APROVITORC,IT.VLRLIQITORC, " +
			 "VO.CODVENDA, VD.DOCVENDA , IVD.VLRLIQITVENDA " + 
			 "FROM  VDORCAMENTO O,VDCLIENTE CL, VDITORCAMENTO IT " +
			 "LEFT OUTER JOIN VDVENDAORC VO ON " + 
			 "VO.CODORC=IT.CODORC AND VO.CODEMPOR=IT.CODEMP AND VO.CODFILIALOR=IT.CODFILIAL AND " +
			 "VO.CODITORC=IT.CODITORC " +
			 "LEFT OUTER JOIN VDVENDA VD ON " +
			 "VD.CODEMP=VO.CODEMP AND VD.CODFILIAL=VO.CODFILIAL AND " +
			 "VD.TIPOVENDA=VO.TIPOVENDA AND VD.CODVENDA=VO.CODVENDA " +
			 "LEFT OUTER JOIN VDITVENDA IVD ON " +
			 "IVD.CODEMP=VD.CODEMP AND IVD.CODFILIAL=VD.CODFILIAL AND " +
			 "IVD.TIPOVENDA=VD.TIPOVENDA AND IVD.CODVENDA=VD.CODVENDA AND " +
			 "IVD.CODITVENDA=VO.CODITVENDA " +
			 "WHERE O.CODEMP=? AND " +
			 "O.CODFILIAL=? AND O.TIPOORC='O' AND " +
			 "IT.CODORC=O.CODORC AND IT.CODEMP=O.CODEMP AND IT.CODFILIAL=O.CODFILIAL AND " +
			 "IT.TIPOORC=O.TIPOORC AND " +
			 "CL.CODEMP=O.CODEMPCL AND CL.CODFILIAL=O.CODFILIALCL AND " +
			 "CL.CODCLI=O.CODCLI" + sWhere +
			 " ORDER BY 2";
	...
	private void consVenda() {
		...
		if (iLin >= 0) {
			if (!tab.getValor(iLin, 0).toString().equals("OV"))
				return;
			try {
				sSQL = "SELECT CODVENDA, TIPOVENDA FROM VDVENDAORC "+
				       "WHERE CODORC=? AND CODEMPOR=? AND CODFILIALOR=?";
				ps = con.prepareStatement(sSQL);
