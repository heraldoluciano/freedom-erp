...
public class FRImpServ extends FRelatorio {
  ...
  public FRImpServ() {
    ...
    rgFormato= new JRadioGroup(1,2,
        new Object[] {"Detalhado","Resumido"},
        new Object[] {"D","R"});
  ...
  public void imprimir(boolean bVisualizar) {
    ...
    int iMesAnt = -1;
  	...
  	try {
  		...
  		if (rgFormato.getVlrString().equals("D")) {
  		  ...
  		  sSql = "SELECT EXTRACT(YEAR FROM V.DTEMITVENDA)," +
  		    	...
  		    	"EXTRACT(YEAR FROM V.DTEMITVENDA) "+
  		    	"ORDER BY 1,2";
  		}
  			...
  			imp.limpaPags();
  			  ...
  			  while ( rs.next() ) {
	  			...
	  			if (imp.pRow()==0) {
	                ...
	                imp.say(imp.pRow() + 0, 34, "Codigo e Razão do cliente");
	            ...
	            imp.say(imp.pRow() + 1, 0, "" + imp.comprimido());
	            imp.say(imp.pRow() + 0, 0, "|"+Funcoes.strZero(rs.getString("CodVenda"), 7));
	            imp.say(imp.pRow() + 0, 10, Funcoes.strZero(rs.getString("DocVenda"), 7));
	            imp.say(imp.pRow() + 0, 19, Funcoes.copy(rs.getString("Serie"),4));
	            imp.say(imp.pRow() + 0, 23, Funcoes.sqlDateToStrDate(rs.getDate("DtEmitVenda")));
	            imp.say(imp.pRow() + 0, 34, Funcoes.copy(rs.getInt("CodCli")+" - "+rs.getString("RazCli"),25));
	            imp.say(imp.pRow() + 0, 60, Funcoes.strDecimalToStrCurrency(10,2,rs.getString("VlrBaseIssVenda")));
	            dTotMesBase += rs.getDouble("VlrBaseIssVenda");
	            imp.say(imp.pRow() + 0, 71, Funcoes.strDecimalToStrCurrency(10,2,rs.getString("VlrIssVenda")));
	            dTotMesISS += rs.getDouble("VlrIssVenda");
	            imp.say(imp.pRow() + 0, 82, Funcoes.strDecimalToStrCurrency(10,2,rs.getString("VlrPisVenda")));
	            dTotMesPIS += rs.getDouble("VlrPisVenda");
	            imp.say(imp.pRow() + 0, 92, Funcoes.strDecimalToStrCurrency(9,2,rs.getString("VlrCofinsVenda")));
	            dTotMesCOFINS += rs.getDouble("VlrCofinsVenda");
	            imp.say(imp.pRow() + 0, 103, Funcoes.strDecimalToStrCurrency(10,2,rs.getString("VlrIRVenda")));
	            dTotMesIR += rs.getDouble("VlrIRVenda");
	            imp.say(imp.pRow() + 0, 114, Funcoes.strDecimalToStrCurrency(10,2,rs.getString("VlrCSocialVenda")));
	            ...
	            imp.say(imp.pRow() + 0, 46, "| "+Funcoes.strDecimalToStrCurrency(15,2,rs.getString(5)));
	            imp.say(imp.pRow() + 0, 64, "| "+Funcoes.strDecimalToStrCurrency(15,2,rs.getString(6)));
      ...
      imp.say(imp.pRow() + 0, 71, Funcoes.strDecimalToStrCurrency(10,2,""+dTotMesISS));
      imp.say(imp.pRow() + 0, 82, Funcoes.strDecimalToStrCurrency(10,2,""+dTotMesPIS));
      imp.say(imp.pRow() + 0, 92, Funcoes.strDecimalToStrCurrency(10,2,""+dTotMesCOFINS));
  ...
  private void impTotGeral(ImprimeOS imp) throws SQLException {
      ...
      imp.say(imp.pRow() + 0, 71, Funcoes.strDecimalToStrCurrency(10,2,""+dTotISS));
      imp.say(imp.pRow() + 0, 82, Funcoes.strDecimalToStrCurrency(10,2,""+dTotPIS));
      imp.say(imp.pRow() + 0, 92, Funcoes.strDecimalToStrCurrency(10,2,""+dTotCOFINS));
