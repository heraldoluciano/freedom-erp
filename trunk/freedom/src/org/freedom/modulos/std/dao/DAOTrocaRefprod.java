...
public class DAOTrocaRefprod extends AbstractDAO {
	...
	private enum QUERY_ORDER {First, Second}
		...
		try {
			...
			while ( rs.next() ) {
				result.addElement( new Table(rs.getString("table_name"), rs.getString("field_name")
						, "S".equals(rs.getString( "emmanut" ))
						, "S".equals(rs.getString( "hcodemp" ))
						, "S".equals(rs.getString( "hcodfilial" )) ) ) ;
			}
			rs.close();
			ps.close();
			getConn().commit();
	...
	public Vector<Change> selectValuesChange(int id) throws Exception {
		...
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			result.add( new Change( rs.getInt( "id_it" ), rs.getInt( "codemppd" ), rs.getInt( "codemppd" )
