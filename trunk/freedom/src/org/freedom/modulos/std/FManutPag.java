...
public class FManutPag extends FFilho implements ActionListener,KeyListener,CarregaListener {
	...
	private void carregaGridBaixa() {
		...
		tabBaixa.limpa();
		try {
			sSQL = "SELECT IT.DTVENCITPAG,IT.STATUSITPAG,P.CODPAG,IT.DOCLANCAITPAG, "+
		...
		try {
			if (validaPeriodo()) {
				...
				if((cbPagas.getVlrString().equals("S")) || 
						...
						(cbPagParcial.getVlrString().equals("S"))) {
					if (cbPagas.getVlrString().equals("S")) {
			...
			if (tabManut.getLinhaSel() > -1) {
				...
				if (!(imgStatusAt == imgPagoParcial || imgStatusAt == imgPago)) {
					if (Funcoes.mensagemConfirma(this, "Deseja realmente excluir esta conta e todas as suas parcelas?")==0) {
						sSQL = "DELETE FROM FNPAGAR WHERE CODPAG=? AND CODEMP=? AND CODFILIAL=?";
						try {
							ps = con.prepareStatement(sSQL);
			...
			if(tabManut.getLinhaSel() > -1) {
				...
				if(((imgStatusAt == imgPagoParcial || imgStatusAt == imgPago))) {
					if (Funcoes.mensagemConfirma(this,"Confirma o estorno do lançamento?")==JOptionPane.YES_OPTION) { 
						...
						iCodPag = Integer.parseInt((String)tabManut.getValor(iLin,5));
						iNParcPag = Integer.parseInt(""+tabManut.getValor(iLin,6));
						sSQL = "UPDATE FNITPAGAR SET STATUSITPAG='P1' WHERE CODPAG=? AND NPARCPAG=? AND CODEMP=? AND CODFILIAL=?";
						try {
							ps = con.prepareStatement(sSQL);
							ps.setInt(1,iCodPag);
