...
public class FRResumoDiario extends FRelatorio {
  ...
  public FRResumoDiario() {
    ...
    txtCodVend.setTipo(JTextFieldPad.TP_INTEGER,8,0);
  	...
  	lcVend.add(new GuardaCampo( txtDescVend, 90, 100, 207, 20, "NomeVend", "Nome do representate", false, false, null, JTextFieldPad.TP_STRING,false),"txtDescVendx");
  ...
  public void imprimir(boolean bVisualizar) {
    ...
    if (rgFormato.getVlrString().equals("D")) {
        sSQL = "SELECT V.DTEMITVENDA,V.CODTIPOMOV,V.CODVENDA,V.DOCVENDA,V.SERIE,"+
                  ...
                  " AND NOT SUBSTR(V.STATUSVENDA,1,1)='C' "+sWhere+" ORDER BY V.DTEMITVENDA,V.DOCVENDA";
//				  " AND TM.TIPOMOV IN ('VD','PV','VT','SE')"+
          ...
          sSQL= "SELECT V.DTEMITVENDA,SUM(V.VLRLIQVENDA) FROM VDVENDA V,"+
				...
				" AND TM.CODTIPOMOV=V.CODTIPOMOV" +
				 (cbVendas.getVlrString().equals("S")?" AND TM.TIPOMOV IN ('VD','SE')":"")+
				" AND V.CODEMP=? AND V.CODFILIAL=? "+sWhere+" GROUP BY V.DTEMITVENDA" ;
