...
//		tabDet.setColunaInvisivel( DETALHAMENTO.CODCOMPRA.ordinal() );
	...
	public void montaGrid() {
		...
		try {
			...
			sql.append( "select " );
			sql.append( "rm.ticket, rm.codtiporecmerc, rm.status, rm.dtins data, rm.hins hora ");
			...
			sql.append( "left outer join eqitrecmercitcp irc ");
			sql.append( "on irc.codemp=rm.codemp and irc.codfilial=rm.codfilial and irc.ticket=rm.ticket ");
			sql.append( "and irc.coditrecmerc=(select first 1 coditrecmerc from eqitrecmerc irc2 ");
			sql.append( "where irc2.codemp=irc.codemp and irc2.codfilial=irc.codfilial and irc2.ticket=irc.ticket ");
			sql.append( "order by irc2.coditrecmerc) ");
			sql.append( "left outer join cpcompra cp ");
			sql.append( "on cp.codemp=irc.codempcp and cp.codfilial=irc.codfilialcp and cp.codcompra=irc.codcompra ");
