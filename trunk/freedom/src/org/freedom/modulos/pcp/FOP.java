...
public class FOP extends FDetalhe implements ChangeListener, PostListener,CancelListener,
		InsertListener,ActionListener,CarregaListener,KeyListener, FocusListener {
	...
	public void geraRMA(){
		...
		try {
			...
			if (rs.next()) {
				try{
					if(temSldLote()){
						if(Funcoes.mensagemConfirma(this,"Confirma a geração de RMA para a OP:"+txtCodOP.getVlrString()+" SEQ:"+txtSeqOP.getVlrString()+"?")
	...
	public void ratearItem(boolean bPergunta){
		...
		try{
			if(bPergunta){
				bResposta = (Funcoes.mensagemConfirma(
								Aplicativo.framePrinc,"Deseja realmente ratear este item para a OP?")
									==JOptionPane.YES_OPTION);
			}
	...
	public void focusLost(FocusEvent arg0) {
		if (arg0.getSource()==txtCodOP) {
			if ( (!((JTextFieldPad) arg0.getSource()).getVlrString().trim().equals("")) 
					&& (txtSeqOP.getVlrString().trim().equals("")) )
