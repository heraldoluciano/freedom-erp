...
public class SafeContabil extends Contabil {
	...
	protected static String getSqlVendasRecebebidas() {
		...
		StringBuilder sql = new StringBuilder();
		/*
		 * Incluído um case para enviar lançamentos de vendas com recibo, nas quais o tipofiscal é igual a 'N'.
		 */
	...
	protected static String getSqlLancamentosAvulcos() {
		...
		sql.append( "SELECT (CASE WHEN S.VLRSUBLANCA>0 THEN P.CODCONTDEB ELSE CT.CODCONTDEB END) CONTADEB, " );
		sql.append( "(CASE WHEN S.VLRSUBLANCA>0 THEN CT.CODCONTCRED ELSE P.CODCONTCRED END) CONTACRED, " );
		sql.append( "(CASE WHEN S.VLRSUBLANCA <0 THEN S.VLRSUBLANCA*-1 ELSE S.VLRSUBLANCA END) VALOR , " );
		sql.append( "' ' SERIE, L.DOCLANCA DOC, " );
		sql.append( "H.TXAHISTPAD, S.DATASUBLANCA DATA, S.CODFILIAL, L.HISTBLANCA AS PORTADOR, " );
		sql.append( "L.HISTBLANCA HISTORICO " );
