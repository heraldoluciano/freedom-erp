...
static void transpose(register struct block *data,
							 register struct ordering *t,
							 register int n);
static void rotate(register struct block *key);
static void f(int i,
				  struct block *key,
				  struct block *a,
				  register struct block *x);
static void setkey(register char *k);
static void myencrypt(char *blck, int edflag);
static int mystrcpy(char *s1, char *s2);
static char * mycrypt(register char *pw, char *salt);

static void transpose(data, t, n)
...
static void myencrypt(blck,edflag)
char *blck;
int edflag;
{
...
register int i;
  ...
  for(i=0;i<25;i++) myencrypt(pwb,0);
  EP=&etr;
  p=pwb;
  pw=result+2;
  while(p<&pwb[66]) {
...
/* As far as I know, we don't use external routines except strcpy
	which is replaced here.  I still don't know why it needs the
   runtime libraries but it requires F_SCOPY@ whatever that is. */
static int mystrcpy(s1, s2)
char *s1;
char *s2;
{
  while (*s2) *s1++ = *s2++;
