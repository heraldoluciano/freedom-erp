...
public class FAtendimento extends FFilho implements CarregaListener, ActionListener {
	...
	private void carregaTabConv() {
		...
		String sSQL = "SELECT ATEND.CODATENDO,ATEND.DOCATENDO,ATEND.STATUSATENDO,ATEND.DATAATENDO,TA.DESCTPATENDO,A.NOMEATEND,ATEND.HORAATENDO" + " FROM ATATENDIMENTO ATEND, ATTIPOATENDO TA, ATATENDENTE A WHERE"
				+ " ATEND.CODCONV=? AND ATEND.CODEMPCV=? AND ATEND.CODFILIALCV=? AND TA.CODTPATENDO=ATEND.CODTPATENDO" + " AND TA.CODEMP=ATEND.CODEMPTO AND TA.CODFILIAL=ATEND.CODFILIALTO" + " AND A.CODATEND=ATEND.CODATEND AND A.CODEMP=ATEND.CODEMPAE AND A.CODFILIAL=ATEND.CODFILIALAE"
