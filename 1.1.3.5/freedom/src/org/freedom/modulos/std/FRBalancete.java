...
public class FRBalancete extends FRelatorio {
	...
	public void imprimir( boolean bVisualizar ) {
		...
		String sSQL = "SELECT P.CODPLAN,P.DESCPLAN,P.NIVELPLAN," + "(SELECT SUM(SL.VLRSUBLANCA*-1)" + " FROM FNSUBLANCA SL,FNLANCA L " + "WHERE L.FLAG IN " + AplicativoPD.carregaFiltro( con, org.freedom.telas.Aplicativo.iCodEmp )
				+ " AND SUBSTR(SL.CODPLAN,1,STRLEN(RTRIM(P.CODPLAN)))=RTRIM(P.CODPLAN) AND " + "SL.CODLANCA=L.CODLANCA AND " + "SL.DATASUBLANCA BETWEEN ? AND ? AND " + "SL.CODEMP=L.CODEMP AND SL.CODFILIAL=L.CODFILIAL "
				...
				+ ( sCodCC.trim().equals( "" ) ? "" : " AND SL.CODCC=" + "(SELECT CC.CODCC FROM FNCC CC WHERE" + " SL.CODEMPCC=CC.CODEMP AND SL.CODFILIALCC=CC.CODFILIAL" + " AND CC.CODFILIAL=? AND CC.CODCC=?)" ) + " AND L.CODEMP=P.CODEMP AND L.CODFILIAL=?)" + " FROM FNPLANEJAMENTO P"
				+ " WHERE P.TIPOPLAN IN ('R','D')" + " AND P.CODEMP=? AND P.CODFILIAL=?" + " ORDER BY P.CODPLAN,P.DESCPLAN,P.NIVELPLAN ";
